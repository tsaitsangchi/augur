...
**位階**〔2026-07-30 機械軌：領域大憲章亦定版為 `docs/系統架構大憲章_v1.54.0.md`；另本檔「憲章」一詞兼指元憲章（`AUGUR-MC v1.6`）與領域大憲章，**引用時一律加前綴以免歧義**〕：系統 doctrine 以 `docs/系統核心思想_v1.10.0.md`（靈魂） + `docs/原則精華_v1.12.0.md` + 憲章為準；
本檔只管「**如何用 AI 工具編輯本專案**」這層短半衰期協作規則。
**語言**：與用戶**所有對話一律繁體中文**（程式碼/識別碼/英文專名可保留原文）。

> **條號前綴紀律〔2026-07-30 機械軌入憲〕**：本檔內裸 `#N` **一律指本檔條號**；引用原則精華之條號**必須寫成 `原則精華 #N`**。原缺口＝多處裸 `#N` 在兩套條號間漂移（如某處「#14」實指原則精華 #14〔經濟價值判定〕，而本檔 #14＝Commit/Push 須明示授權；同型於 #11 之「經濟終關」、#17 之「principle→factor_map→#14」、#28 之「三敵人零容忍（#1／#8／#15）」）——**讀者依裸號跳錯條即誤解義務**。存量之逐處補前綴列殘餘待辦；本紀律自即日適用於新增文字。

> **條號導讀**：條號按「**新增順序**」編、按「**分類**」歸章，故**非連續**——一 `#1-8`（通用）· 二 `#9-12, 32`（資料真實）· 三 `#13-20, 29, 35`（編輯規則）· 四 `#21-25, 28, 30, 31, 33, 34`（long-running）· 五 `#26-27`（協作模式）。
...
32. **AI 自身宣稱走同一條路（`AUGUR-MC v1.6 §P4.E7`／`AUGUR-L6 v1.2` L6.4；大憲章 v1.51.0「普遍晉升路徑」之工具層落點）〔v1.32 入憲〕**：
    - **(a) self-reported 標記**：AI 產出之文字／評分／裁決意見一律標記為 self-reported，**不得作為「世界如此」或「能力如此」之權威確認**（#9 之三類來源皆屬系統自身通道，故不因「有程式輸出」而免此標記；**系統自身 LLM／agent 之自陳輸出不屬 #9 白名單**，見原則精華 #15）。
    - **(b) 能力宣稱須過預凍對照臂**：任何「某模型／某流程有某能力」之宣稱，須先跑**預先凍結**之對照臂（至少：**常數／樣板地板臂、上限臂、錯配臂**）並**公布三臂分數**；未嚴格勝過地板者一律回報「無證據」，**不得以單臂高分宣稱能力**（實證教訓：常數字串曾得 0.654 > 冠軍 0.492）。
    - **(c) 判死留檔**：被證偽之能力宣稱與其量尺一併留檔、不得靜默消失；失敗回流為下一輪量尺題材。
    - 原缺口＝行走者④「AI 能力宣稱」在工具層無預先凍結判準、無獨立證據通道、無判死留檔之明文。
...
20. **計畫先行（plan-first，憲章第六部 v1.33.0；SSOT 在該條）**：本專案**一切規劃類工作**（新功能／子系統／介面／架構／流程／判準變更、及 ≥3 步或破壞性任務）**一律先產出計劃報告（寫 `reports/`，#16 命名）＋用戶拍板後才實作**——**規劃前先把 what/why 理解窮盡**（分級：預設單視角自審＋完整性對抗自問，僅觸高風險門檻〔治權判準/跨≥2治權檔、不可逆、安全授權邊界、跨≥3 package〕才多 agent 平行對抗審查＋附發現表留痕，依 #34 該開就開滿、#28(a) 本地優先仍第一順位）、不邊想邊做、不先寫 code 再補計畫。報告須達憲章「計畫完整性」（**v1.39.0 強化:所有計畫書〔含純分析〕一律附 (a) 對應 table schema〔產表附新表 DDL、不產表附所讀既有表 schema＋結果落哪張表〕＋ (b) 對應 python 程式規畫〔建表·遷移·消費·強制之檔·函式·角色;純分析亦列所有 script 職責·簽名·輸入輸出表〕,表與程式雙落實**;另附元件・端點・分階段・驗收）＋30 分鐘可讀。**執行前確認**：拍板後、動工前主動確認計劃①完整②內部一致③與現況/code 一致④可實作，有落差先修計劃再執行（承 §26 不讓用戶當 QA）。**判準＝交付物本質／比例原則**：屬「規劃」→ 計畫先行；純機械修正（改錯字／補一欄／既定計畫逐步落地）不另立計畫、不須多視角。SSOT＝憲章第六部「計畫先行」。
 - **治理提案不設公示要件（Steward 指示 2026-07-23 入憲 v1.31）**：本專案為 **Sole Steward 專案（由 Steward 定義）**——AI 草擬任何治理提案／裁決／計畫時，**不得再新訂「須公示（如 14 日）方生效」之類要件**。本句為 [I] 工具層慣例（管未來草擬行為），不變更憲章／治理附則既有 [N] 文字。**既有附則第 2 條公示條款之存廢已拍板（甲案，2026-07-23）**：強制公示要件經附則 minor 修訂廢止（公示改任意；`RULING-2026-031`、AL-2026-034，處置記錄 `reports/augur_steward_no_public_notice_directive_20260723.md`）——**GOV-4 反向閉合**（不錨 [N]、不再列開放待辦）；`PROPOSAL-2026-001`（僅 GOV-1）已於 2026-07-23 依 §8.5(b) 正式議決通過生效（AL-2026-035：AUGUR-MC v1.4→v1.5、§8.1 增「解釋之界線」段；MC 現行 v1.6）。
...
29. **Script 個別可執行 × 資料驅動不 hardcode（用戶 directive 2026-07-02 入憲）**：`scripts/` 每支程式須滿足四件事——
    ...
    - **(b) 資料驅動、來源住 DB、不 hardcode 資料（v1.15 升級,用戶 directive 2026-07-02:repo JSON 檔=另一種 hardcode）**：策展/擷取資料一律住**本地 PostgreSQL**,以**三層知識管線**運作（鏡射 raw→下游）——**來源定義表 `knowledge_source`**（adapter+查詢模板 registry）→ `acquire_knowledge.py`（通用擷取引擎,從外部真實來源抓入）→ **暫存表 `knowledge_staging`**（payload JSONB+provenance,#1 可溯源,pending 待審）→ `promote_knowledge.py`（晉升引擎,審核後冪等寫正式表）;批次驅動＝`harvest_knowledge.py` 排程矩陣（`knowledge_query`×來源、resume 帳本）。
      ...
      - **別名/映射/config 類「決定行為的資料」亦住 DB（v1.19,用戶 directive 2026-07-07:`TOPIC_ALIAS` 寫死違此）**:凡「策展的、會增減、非邏輯」之映射/別名/分類/來源對照（如 topic→domain 別名 `knowledge_topic_alias`、CJK→EN 檢索詞表 `retrieve_glossary`、蒸餾種子題 `advisor_distill_seed_topic`〔v1.49.0 入憲〕）**一律住 DB 表、不得寫死 Python dict/list**;新增映射＝admin INSERT 一列、零改碼。**判準:「這是資料（策展、會增減、可外部產生）還是邏輯（演算法/型別/流程）?」——資料→住 DB;邏輯→留 code**。種子可一次性 bootstrap 遷移（等同 knowledge_source 種子）,但 SSOT 是 DB 表非 code。**豁免**:安全繫於機械閘、非資料鎖之品質工程詞表（如 guard 之 `safe_general` 白名單,憲章 v1.35.0 已裁「詞表不鎖＝執行層、安全繫於 guard 非詞表」）屬邏輯側,不在此列。
...
35. **回歸鎖三規則（Steward 拍板 2026-08-01 入憲 v1.35；限向前生效——僅課「新寫或改寫」之保護斷言，存量以基線治理、不課回溯債）**：凡為保護某行為／不變式而寫之測試、`--selftest` 斷言、guard 接線驗證（合稱**回歸鎖**），於**新寫或改寫**時一律守三規則——**純函式餵真輸入／下游絆線／禁字面斷言——凡新回歸鎖必先驗紅**。
...
33. **禁止阻塞等待迴圈 × 會話注意力不得用於等待（用戶 directive 2026-07-31 入憲；#21 之對偶）**：
    **嚴禁**於前景工具呼叫中以迴圈等待外部作業完成——包含但不限於
    `until <cond>; do sleep N; done`／`while ! <cond>; do sleep N; done`／`wait $PID`／
    連鎖 `sleep` 疊加／任何以輪詢達成「等到好為止」之寫法。
    - **為何是硬禁**：背景作業本身**不佔** AI 之注意力，佔住的是那個迴圈。一旦寫下去，
      AI 直到逾時前**對用戶完全失去回應**——用戶會誤判為「作業影響了 AI」，實則是工具用法選錯。
      （實犯 2026-07-31：`until ! pgrep -f "pg_dump…"; do sleep 15; done` 卡滿 600s 逾時，
      期間用戶連發三則訊息無人回應；用戶問「為什麼 dump 會影響 claude 同步」——**根因非 dump，是本迴圈**。）
    - **正解三選一**：(a) `run_in_background` 啟動，完成時由 harness 通知（**首選**，承 #28(b)「背景監看、不輪詢」）；
      (b) 隔次查詢——每輪工具呼叫只查一次狀態即返回，讓用戶得以插話；
      (c) 真需等待之短暫前置（如服務重啟後探埠）以**單次**短 `sleep`（≤10s）為限，不得成迴圈。
    - **判斷句**：「**這個指令會不會讓我在它跑完之前無法回應用戶？**」會 → 改背景或改隔次查詢。
    - **邊界**：本條管 AI 之工具呼叫寫法，**不禁**背景作業自身之內部等待
      （如 `run_evolution_iteration --slot-wait`、`flock -w`——那些跑在背景、不佔會話）。
...
28. **Claude usage 經濟原則 + 配額護欄（批量/長跑通用；#24 之 Claude-配額對偶）**：批量 / 長跑作業（workflow、多 agent、過夜 sync、逐特徵 build）須**配額感知**——近額度上限即暫停、過重置點再續，**絕不在配額用盡時被硬切於半途**留下不一致狀態。#24 防的是**外部 API（FinMind/FRED）**過載；本條防的是 **Claude 自身 usage 配額**耗盡，同一精神。
    - **最小化 usage 為總則（用戶 directive 2026-06-27）**：**本專案所有批量處理之設計與執行一律以「最小化 Claude usage 消耗」為原則**——(a) **本地優先**：能用本地 DB / Python / script 算的（build / sync / 對帳 / 大量查詢）不繞道 Claude model / subagent（本地計算零 usage）；(b) **harness 背景監看、不輪詢**：長跑用背景模式（完成自動通知），不主動反覆查狀態、不自掛喚醒鏈；~~(c) **非必要不 fan-out**~~〔**v1.34 由 #34 反向廢止**：確定要用 Claude 之工作平行度一律拉滿；(a) 本地優先仍為第一順位不變〕；(d) **批次 / 向量化優於逐項**；(e) **回報精簡**。
      ...
      - **模型檔位分派（v1.27,用戶 directive 2026-07-13；二分之模型對映）**：Claude Code 模型選擇沿用同一裁決句——「**搞錯會沉默污染下游 → 高檔位；僅僅是慢 → 低檔位**」。分派表：
        ...
        **切換紀律**：`/model` 切換後新模型須全量重讀 context（prompt cache 不跨模型）——**同檔位作業湊一批做完再切、不逐訊息 ping-pong**；fan-out 時 subagent 個別指定低檔模型即可、主 session 不必降檔。**優先序**：模型檔位居 #28 手段之末——(a) 本地零 usage ＞ (b) 背景不輪詢 ＞ 模型檔位；本地審議引擎可裁域仍先於任何 Claude 檔位。〔**原 (c)「不 fan-out」已於 v1.34 由 #34 反向廢止**——確定要用 Claude 者平行度拉滿；本序其餘不變。〕
...
34. **Claude 平行度開到最大（Steward directive 2026-07-31 入憲；本條明示修正 #28(c)）**：
    凡確定要用 Claude 做的工作，**平行度預設拉滿、不自行保留餘裕**——不再以「省配額」為由縮減
    agent 路數、不再把大題拆成幾輪序列跑、不再因「兩個 workflow 會不會互相拖垮」而延後啟動第二個。
    - **明示修正**：#28(c)「**非必要不 fan-out**」自本條起**反向**——改為「**該用 Claude 就開到滿**」；
      #28 之優先序末段「(c) 不 fan-out」同步作廢（見該條已改文字）。**#28 其餘各款不變**，特別是
      (a) 本地優先仍為**第一順位**：一句 `psql` 能得的數字仍走 `psql`，不派 agent 去查——
      **本條放寬的是「已經確定要用 Claude 的那部分要開多寬」，不是「什麼都改用 Claude」**。
    - **實測依據（#15，2026-07-31 本機 PC002-S1800）**：3 個 workflow ≈ 27 個並行 agent 同跑時，
      12 執行緒之 load average ＝ **1.96**（未達單核滿載）。**agent 是網路 I/O bound、不是 CPU bound**——
      原先「怕互相拖垮」之前提經量測**不成立**。續調更高並行時仍依 #27 逐級逼近、以實測退場。
    - **操作值**：單一 workflow 之並行上限＝ `min(16, 核數-2)`（本機＝**10**），**預設把路數開到該上限**；
      多個 workflow 可同時在跑（各自獨立計上限）。路數不足以填滿時才減。
    - **不因本條鬆動者（三項硬邊界）**：
      (i) **配額護欄**（#28）——撞 429／weekly／session limit 仍**立即停、不 retry 硬衝**；
      (ii) **OCV 單向棘輪**（#26）——加寬平行度**不得**減少人類介入點數／否決可達性／揭露比例；
           每個 workflow 仍須回報主 session、仍不得自我生效，**寬度增加、人閘不變**；
      (iii) **#33 禁阻塞等待迴圈**——並行變多更不得以輪詢等待收斂，一律背景通知或隔次查詢。
    - **原缺口**：#28(c) 之措辭使 AI 在「不確定要不要 fan-out」時一律偏保守，
      實犯 2026-07-31＝以「避免兩個大 workflow 互相拖垮」為由延後第二個 workflow，
      而**該前提從未量測**——用戶連問三次「你不能增加同步嗎」才更正。
...
26. **有界自主推進 / 自驅動（授權後主動、碰護欄停手；＝原則精華 #20 之工具層落地）**：
    - **授權四要件（`AUGUR-MC v1.6 §P5.W2`／`AUGUR-L6 v1.2` L6.5–L6.8）〔v1.32 入憲〕**：每次有界自主授權須明示 **(a) 範圍**（可為之工作類別與護欄）、**(b) 有效期限或結束條件**（**逾期自動失效，續跑須重新授權**）、**(c) 可撤銷**（隨時收回）、**(d) 所繫之計畫／任務參照**；四項連同**授權時點**記入該次工作之報告或任務清單，作為**授權鏈留痕**（Observation）。**無期限、無範圍、無留痕之概括授權不成立**——遇此情形 AI 須先請用戶補明四要件，或自限於單次可逆工作。（原缺口＝人類權威根節點以口頭、無期限、無留痕方式授出，事後無從機器稽核「誰授權、授到哪、何時失效」。）
    - **自動鏈上限（OCV C／D 分量之工具層落點；`AUGUR-L6 v1.2` L6.16–L6.17）〔v1.32 入憲〕**：**(a)** 每次授權須載明本輪**最大連續無人介入圈數／時長上限**，達上限即**停並回報**、等待人類續授；**(b)** 凡新增或延長背景編排、排程、自我喚醒鏈者，須於報告中對照前後之**「人類介入點數、否決可達性、揭露比例、最大自動鏈長」**四項並聲明未弱化（單向棘輪），**任一弱化即屬治權變更、停下問**。（原缺口＝全檔唯一對自動鏈設限之句〔#28 不自掛長喚醒鏈〕理由是省配額而非人類監督，OCV C 分量在工具層無落點。）
經用戶授權，AI 得不逐步等指令、**自己 prompt 自己（loop）**自主朝**靈魂目標**（SSOT＝`docs/系統核心思想_v1.10.0.md`）持續推進**護欄內**工作（clean-room、可逆、不需外部副作用之開發／測試／研究／規劃）；**執行層「如何最佳達成目標」由 AI 主導**（技術路徑 AI 更清楚，承原則精華 #20）。
    ...
    精神承靈魂「系統建議、人決策；**有紀律的顧問**——**不是自動駕駛**＝禁改治權判準／禁自動下單／人得緊急停；**PME-AUTO-B** 閘內狀態晉升屬執行層」——**決策層人拍板、執行層 AI 主動**，同樣適用於開發 augur 本身。授權可隨時收回。
