# CLAUDE.md — Augur AI 協作工具規則 v1.17
...
20. **計畫先行（plan-first，憲章第六部 v1.32.0；SSOT 在該條）**：本專案**一切規劃類工作**（新功能／子系統／介面／架構／流程／判準變更、及 ≥3 步或破壞性任務）**一律先產出計劃報告（寫 `reports/`，#16 命名）＋用戶拍板後才實作**——**規劃前先把 what/why 理解窮盡**（分級：預設單視角自審＋完整性對抗自問，僅觸高風險門檻〔治權判準/跨≥2治權檔、不可逆、安全授權邊界、跨≥3 package〕才多 agent 平行對抗審查＋附發現表留痕，守 #28「非必要不 fan-out」）、不邊想邊做、不先寫 code 再補計畫。報告須達憲章「計畫完整性」（產表附 schema＋程式；非產表附元件・端點・檔案・分階段・驗收）＋30 分鐘可讀。**判準＝交付物本質／比例原則**：屬「規劃」→ 計畫先行；純機械修正（改錯字／補一欄／既定計畫逐步落地）不另立計畫、不須多視角。SSOT＝憲章第六部「計畫先行」。
