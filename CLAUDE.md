# CLAUDE.md — Augur AI 協作工具規則 v1.2

**性質**：AI（Claude 等）在本專案編輯/執行時的工具規則。
**位階**：系統 doctrine 以 `docs/系統核心思想_v1.1.0.md` + `docs/原則精華_v1.5.0.md` + 憲章為準；
...
17. **Clean-Room 重建（SSOT＝原則精華 #16，本條僅工具層引用）**：augur 所有程式產生一律 **clean-room**——只依 5 治權檔（靈魂 / 原則精華 / 憲章 / CLAUDE.md / README）+ augur 自身 schema 目錄 + live API 實證 建立；**產生任何 code 時，不讀、不參考、不移植 stock_backend 之任何 code / 資料 / 報告 / 數字 / 設定**（唯一 sanctioned 觸點＝憲章附錄 B 考古／已抽象之思想啟發，二者**不得回流 code**）。碰 ingestion/feature/universe/model 時對照 `docs/原則精華_v1.5.0.md`（source-pure / anti-leakage / 型別 / SSOT…）；不確定先查靈魂與憲章。
...
26. **有界自主推進（授權後主動、碰護欄停手）**：經用戶授權，AI 得不逐步等指令、自主朝**靈魂目標**（SSOT＝`docs/系統核心思想_v1.1.0.md`）推進**護欄內**工作（clean-room、可逆、不需外部副作用之開發／測試／研究／規劃）。
    ...
    - **以真兆為據（#15）**：自我糾錯依 probe／實測／API 事實，**不自我說服「我覺得對」**（防「以為懂其實自欺」——主動 ≠ 自信犯錯）。
...
27. **可控風險下逐級逼近最佳奇異點 / 試錯即進步（對齊原則精華 #19；augur 解問題之通用做法）**：經用戶授權探索某參數/解法之最佳點（FinMind throttle rate / 並發 / 任何可調項）時——
    ...
    - **重覆驗證再定論、單次不入憲**：新 operational 安全值須重覆實證 + source-traceable 才寫治權檔（#9/#10）；未驗證前既有安全值（`1.0s`）維持為操作值，程式內實驗值須註明「實驗中」。**不怕一時的可恢復錯誤——進步來自試錯**。
