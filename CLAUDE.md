...
20. **計畫先行（plan-first，憲章第六部 v1.31.0 入憲）**：本專案**一切規劃類工作**（新功能／子系統／介面／架構／流程／判準變更、及 ≥3 步或破壞性任務）**一律先產出計劃報告（寫 `reports/`，#16 命名）＋用戶拍板後才實作**——不邊想邊做大改、不先寫 code 再補計畫。報告須達憲章「計畫/設計文件完整性」（產表附 schema＋Python；介面/流程附元件・端點・檔案・分階段・驗收）＋30 分鐘可讀。**判準＝交付物本質**：屬「規劃」（決定做什麼／怎麼架構）→ 計畫先行；純機械修正（改錯字／補一欄／既定計畫逐步落地）不另立計畫。SSOT＝憲章第六部「計畫先行」。
