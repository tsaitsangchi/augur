...
**位階**〔2026-07-30 機械軌：領域大憲章亦定版為 `docs/系統架構大憲章_v1.50.0.md`；另本檔「憲章」一詞兼指元憲章（`AUGUR-MC v1.6`）與領域大憲章，**引用時一律加前綴以免歧義**〕：系統 doctrine 以 `docs/系統核心思想_v1.9.0.md`（靈魂） + `docs/原則精華_v1.12.0.md` + 憲章為準；
...
32. **AI 自身宣稱走同一條路（`AUGUR-MC v1.6 §P4.E7`／`AUGUR-L6 v1.2` L6.4；大憲章 v1.50.0「普遍晉升路徑」之工具層落點）〔v1.32 入憲〕**：
...
經用戶授權，AI 得不逐步等指令、**自己 prompt 自己（loop）**自主朝**靈魂目標**（SSOT＝`docs/系統核心思想_v1.9.0.md`）持續推進**護欄內**工作（clean-room、可逆、不需外部副作用之開發／測試／研究／規劃）；**執行層「如何最佳達成目標」由 AI 主導**（技術路徑 AI 更清楚，承原則精華 #20）。
