...
> **憲章從屬（AUGUR-MC v1.3）**：本文件為 Augur 領域治權文件，受 **AUGUR-MC v1.3** 約束（Layer 0 lex superior，§0.6）。依 Steward 裁決第 2026-002 號（AL-2026-006）登錄為 **Layer 6（Agent Runtime）**。下層引用格式：`AUGUR-MC v1.3 §{條款}`／`AUGUR-WM v1.0 §{條款}`。憲章與規格存於 [tsaitsangchi/augur](https://github.com/tsaitsangchi/augur) 之 `constitution/`／`specs/`（原獨立倉已併入）。合規聲明補正期至 2026-10-14。
