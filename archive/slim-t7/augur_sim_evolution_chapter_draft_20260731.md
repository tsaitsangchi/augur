# 【草案】模擬方法與自進化元件專章——普遍晉升路徑之本軸門檻

> ## ⚠ 本文為**草案**，尚未生效
>
> * **性質**：依 `AUGUR-MC v1.6 §8.1`（Agent 不得參與修憲與解釋）與 `RULING-2026-028` 第 2 點
>   （**繕打／依核示落地＝非參與**），本文係 claude 依 Steward 2026-07-31 明示指示
>   「**現在就把專章草案寫出來**」所為之**繕打**。**實質判斷之作成屬 Steward**。
> * **未生效**：本文任何條文在 Steward 議決前**不具效力**；不得被引用為已成文之法。
> * **三處待裁**：全文有三個 **【待裁 ①②③】** 空格，Steward 選定後條文方為完整。
...
> ### 【待裁 ①】本軸之終審採何級？
>
> 總則節點 2 括號原文：「**若該類地位在本質上無經濟對價，須於其專章明文宣告以何為終審，
> 並載明其為統計級而非實效級**」。
>
> - **甲（繕打者建議）**：本軸終審＝**統計級——校準檢定**（實現值落入預測分位錐之
>   覆蓋率與 PIT 均勻性），**並於此明文載明：本軸之終審為統計級，非實效級 `#14` 經濟終關**。
>   *理由*：本軸產物為「風險形狀」，**依 §1.2 本就不得產生可交易之方向或點位**，
>   故無經濟對價可資裁決；若強行套 `#14`，只會逼出「為過經濟關而偷偷做方向」之壓力，
>   反而侵蝕 §1.2 之邊界。
> - **乙**：仍走 `#14` 經濟終關。
>   *代價*：需先定義本軸如何產生可交易訊號——**而那與 §1.2 直接衝突**。
>
> **選定：☐ 甲　☐ 乙　（Steward 簽註：____________　日期：________）**
...
>
> - **甲′（繕打者修正後之建議；原「甲」已因實查事實二、三而不足）**：
>   接受**偵測級**，但**須併同下列三項最低補強，否則本軸之人閘自始無效**：
>   1. **`governance_queue.py` 不得以 `getpass.getuser()` 自動取簽名者**——
>      改為要求 TTY 互動輸入（比照既有三支之 `isatty` 先例），
>      非 TTY 環境一律拒絕寫入而非自動代填；
>   2. 本軸之工具**一律不設人名旗標**，且 selftest **不得寫入任何人簽欄**
>      （既有三筆 `selftest`／`hugo-authorized-selftest` 即此紀律缺位之實證）；
>   3. 本條**明文承認其為偵測非預防**，不偽稱已機械封閉。
>   *理由*：可即刻生效；且誠實標明射程，優於宣稱有閘實則無。
>   **但須認知：在補強 1 完成前，「人簽」欄位之證據力接近於零**——
>   因為它可能是任何以該 OS 帳號執行之行程所填。
> - **乙**：先做**角色分離**（治權表改由非 `augur` 角色持有並 REVOKE）達預防級。
>   *代價*：影響所有 script 之連線角色，屬破壞性變更（`CLAUDE #6`），須另案；本軸實作延後。
>
> **選定：☐ 甲′　☐ 乙　（Steward 簽註：____________　日期：________）**
...
>
> - **甲**：擴兩處 CHECK 各加 `'sim'`。*代價*：把不一致再複製一次；日後新增軸仍須改碼。
> - **乙（繕打者建議）**：改為 **axis registry 表**（`evolution_axis`），兩處 CHECK
>   改為 FK。*理由*：守 `CLAUDE #29(b)`「**決定行為的資料住 DB、不寫死**」——
>   軸是**策展的、會增減、非邏輯**之資料；且一次消除兩表不一致。新增軸＝INSERT 一列、零改碼。
>   *代價*：多一張表與一次遷移；既有兩表須先對齊值域。
>
> **性質認定保留**：本項變更屬 patch 或重大判準修正，依 `GOVERNANCE-ANNEX v1.1`
> 第 2 條第 3 款「**patch 之性質認定屬 Steward 保留、Agent 不得自行認定**」，**繕打者不予認定**。
>
> **選定：☐ 甲　☐ 乙　（Steward 簽註：____________　日期：________）**
> **性質認定：☐ patch　☐ 重大判準修正　（Steward 簽註：____________）**
