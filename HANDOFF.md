...
| **PME-XDOM 近程** | ✅ **開通＋S0／S1 文獻橋**（2026-07-28；`PME-XDOM-YES`＋`PME-XDOM-SUNZI-MGMT`＋`GATE-keep`＋`FZ-keep`；拍板＝`audits/PME-XDOM-PLAN-APPROVED-20260728.md`；S0＝`reports/augur_pme_xdom_sunzi_mgmt_s0_20260728.md`；S1＝`scripts/curate_pme_xdom_map.py`；收官＝`audits/PME-XDOM-SUNZI-MGMT-S01-CLOSED-20260728.md`）——異域概念經 **investment** 假說鏈；**ERP dump 不自動灌**；**未**跑 S3／S4（**待開 `PME-XDOM-S3`**）；≠可交易／≠解凍 |
...
| **RKI 近程** | ✅ **S01 CLOSED**（2026-07-28；`RKI-PLAN`＋`RKI-SCOPE-ALL-KH`＋`RKI-S01`＋`FZ-keep`＋`NHC-keep`；含同日追加 AI×預測／FP×AI 迭代／FP×投資預測迭代）；拍板＝`audits/RKI-PLAN-APPROVED-20260728.md`；收官＝`audits/RKI-S01-CLOSED-20260728.md`；表 `knowhow_interaction_probe` **active=12**；S0＝`reports/augur_rki_s0_inventory_20260728.md`；**待** `RKI-S2`／`RKI-S3`；**`PME-XDOM-AI-PREDICT` 另拍**；計畫＝`reports/augur_raw_knowhow_interaction_probe_plan_20260728.md` |
| **不做（近程）** | **其餘**異域進化閉環（太陽能↔儲能、`PME-XDOM-AI-PREDICT` 等）仍鎖除非另拍；孫子↔**ERP dump** 自動 map 仍禁；不以顧問 cite 率當 G-PROM 通過；不降閘／不解凍 |
