...
| `docs/系統架構大憲章_v1.48.0.md` | **憲法**：三敵×管線、12-PHASE、升版規則、**知識一律准入＋漸進 KH（v1.48.0）**、第六部計畫先行、修訂歷程 |
...
| **PME-XDOM 近程** | ✅ **開通＋雙軸 S0／S1＋SUNZI S3**（S01＝2026-07-28；S3＝2026-07-29 Steward「所有 working 開始跑」＝開 `PME-XDOM-S3`＋GATE／FZ-keep）——①孫子×企管：`audits/PME-XDOM-SUNZI-MGMT-S01-CLOSED-20260728.md`＋**S3**＝`audits/PME-XDOM-SUNZI-MGMT-S3-CLOSED-20260729.md`（local-gates **run_id=6**；異域新 map **零雙綠**；active n=1＝`inst_cumflow_position_120d`；S2 缺特徵未建）②AI-PREDICT：S01＝`audits/PME-XDOM-AI-PREDICT-S01-CLOSED-20260728.md`；**S3 可重用 run6**（競態 defer；孤兒 run5 另清）；**S4 不建議急開**；ERP dump 不自動灌；≠可交易／≠解凍 |)
...
| **不做（近程）** | **其餘**異域進化閉環（`PME-XDOM-SOLAR` 等）仍鎖除非另拍；孫子↔**ERP dump** 自動 map 仍禁；不以顧問／RKI cite 率當 G-PROM 通過；不降閘／不解凍；SUNZI **S3 已 CLOSED**／**S4 未開**；AI-PREDICT **S3 另收斂**（可重用 run6）；**KNI-S2／S3 未開**；ADM-AI-ASSIST **timer 預設不 apply**（須顯式） |
...
| 判準/憲法 | `docs/系統架構大憲章_v1.48.0.md`＋`docs/原則精華_v1.10.0.md`＋`docs/系統核心思想_v1.8.0.md` |
