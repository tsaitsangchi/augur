...
### 4.4 紅線（絕不能做）
- ⚠ **操作凍結（2026-07-24；同日收緊）**：**FinMind／FRED 外部 API 一律不開**，直至 **憲章→實作路線圖（constitution-to-implementation）全部階段落地完成之後**，且用戶明示解凍（「解凍 FinMind／FRED」等）——含 sync／probe／放量／窄窗／Dividend 重建；**「計畫落地」／R5 S1–S2／局部階段完成 ≠ 解凍**；護欄＝`.cursor/rules/finmind-fred-api-freeze.mdc`（alwaysApply）。允許本地 DB 唯讀／零網路／計畫／免 API pytest／零 API 實作。
