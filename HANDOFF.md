...
| `docs/系統架構大憲章_v1.47.0.md` | **憲法**：三敵×管線、12-PHASE、升版規則、**第六部計畫先行/計畫完整性 v1.39.0**、修訂歷程 |
...
| **PME-XDOM 近程** | ✅ **開通＋雙軸 S0／S1**（2026-07-28；①`PME-XDOM-YES`＋`SUNZI-MGMT`＋`GATE-keep`＋`FZ-keep`→`audits/PME-XDOM-SUNZI-MGMT-S01-CLOSED-20260728.md`；②**`PME-XDOM-AI-PREDICT`**＋GATE／FZ／NHC-keep→拍板＝`audits/PME-XDOM-AI-PREDICT-APPROVED-20260728.md`；S0＝`reports/augur_pme_xdom_ai_predict_s0_20260728.md`；S1＝`scripts/curate_pme_xdom_ai_predict_map.py`；收官＝`audits/PME-XDOM-AI-PREDICT-S01-CLOSED-20260728.md`；school `ml_predict_evolution`＋map×10；**未**跑 S3／S4——**待開 `PME-XDOM-AI-PREDICT-S3`**；ERP dump 不自動灌；≠可交易／≠解凍） |
| **NHC 近程** | ✅ **S12 CLOSED**（2026-07-28；`NHC-PLAN`＋`NHC-S12`＋`FZ-keep`；**無** `NHC-CONSTITUTE`）；拍板＝`audits/NHC-PLAN-APPROVED-20260728.md`；收官＝`audits/NHC-S12-CLOSED-20260728.md`——`retrieve_glossary` active=13（漿料 cooccur）；`query_translation` 讀表、無 `_GLOSSARY` SSOT；A0 四探針（應用／技術核心／物理／化學）走通用 `advise`、禁領域 hardcode；**待** `NHC-S3`／`NHC-CONSTITUTE`；計畫＝`reports/augur_no_hardcode_db_ssot_constitution_plan_20260728.md` |
| **RKI 近程** | ✅ **S01 CLOSED**（2026-07-28；`RKI-PLAN`＋`RKI-SCOPE-ALL-KH`＋`RKI-S01`＋`FZ-keep`＋`NHC-keep`；含同日追加 AI×預測／FP×AI／FP×預測迭代／**AI×太陽能研發**）；拍板＝`audits/RKI-PLAN-APPROVED-20260728.md`；收官＝`audits/RKI-S01-CLOSED-20260728.md`；追加＝`audits/RKI-AI-SOLAR-RD-SEED-20260728.md`；表 `knowhow_interaction_probe` **active=14**；S0＝`reports/augur_rki_s0_inventory_20260728.md`；**待** `RKI-S2`／`RKI-S3`；**`PME-XDOM-AI-PREDICT` 已另拍並 S01 CLOSED**（探針仍≠過閘憑據）；**`PME-XDOM-SOLAR` 仍另拍**；計畫＝`reports/augur_raw_knowhow_interaction_probe_plan_20260728.md` |
...
| **ADM-AI-ASSIST 近程** | ✅ **S01 CLOSED**（2026-07-28；`ADM-AI-ASSIST-PLAN`＋`FZ-keep`）——schema **選項 C** `knowledge_admission_assist`；`assist_admission_review.py` 預設 dry-run；**硬禁** AI／timer approve／activate；**待另令** S2 有界 apply／gov 唯讀、S3 timer；拍板＝`audits/ADM-AI-ASSIST-PLAN-APPROVED-20260728.md`；收官＝`audits/ADM-AI-ASSIST-S01-CLOSED-20260728.md`；計畫＝`reports/augur_ai_admission_assist_plan_20260728.md` |
| **不做（近程）** | **其餘**異域進化閉環（`PME-XDOM-SOLAR` 等）仍鎖除非另拍；孫子↔**ERP dump** 自動 map 仍禁；不以顧問／RKI cite 率當 G-PROM 通過；不降閘／不解凍；AI-PREDICT／SUNZI **S3 未開**；**KNI-S2／S3 未開**；**ADM-AI-ASSIST S2／S3 未開** |
...
| 判準/憲法 | `docs/系統架構大憲章_v1.47.0.md`＋`docs/原則精華_v1.10.0.md`＋`docs/系統核心思想_v1.8.0.md` |
