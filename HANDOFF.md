...
**2026-07-12 深夜保全點(滑鼠故障,可能重開機)**:①K 計畫六件全執行(K1 橋 59,706 係數/K2 items 軌 119,764/K3 三閘+四工具+oai_pmh/K4 timer/R6 合格 0.87/O1 生產器 fail-closed 實證);②**重開機後須重啟的兩件**:(a)三鏡頭月頻建置(背景 nohup,resume-safe):`cd ~/project/augur && nohup venv/bin/python scripts/build_threelens_monthly.py --run >> ~/threelens_build.log 2>&1 &`(進度看 log,PK 冪等續跑);(b)audit 自癒跑者:`nohup flock -n /tmp/augur_audit.lock bash <scratchpad>/audit_when_healthy.sh >> ~/audit_retry.log 2>&1 &`——但 scratchpad 隨 session 清,重開機改直跑 `FINMIND_MIN_INTERVAL=2.0 venv/bin/python scripts/daily_maintenance.py --audit-since 2026-06-01`(IP 已於 16:42 癒合、2.0s 退級步調);systemd timers(embed-catchup/green/knowhow-refresh)與六服務開機自起、不用管;③待 hugo:R4 approve×2(--actor admin)、FRASER_API_KEY 落 .env(格式:無空格無括號)、三鏡頭 T2/T3 待建置完成後由 AI 續。

**2026-07-12 晚間二段(封存點 archive-20260712-pregame-allsigned;重開機後 session)**:①災後恢復:六服務+timers 開機自起全綠、DB 零殘鎖;三鏡頭月頻建置重開機前已完成(112/112,2,229,083 值/2017-01→2026-05);**audit 自癒跑者改住 `~/augur_audit_selfheal.sh`(持久、取代已亡 scratchpad 版;重啟=`nohup flock -n /tmp/augur_audit.lock bash ~/augur_audit_selfheal.sh &`,log=~/audit_retry.log,探測先行+PYTHONUNBUFFERED)**;②FRASER key 有效化:FRASER 僅收 **X-API-Key header**(query param 必 401)→ 新 `adapter_config.auth_header={"name","env"}` 機制(probe+acquire 皆支援、key 不進 URL 不落 log);review_log 既洩 key×2 已遮蔽 `<redacted>`;③R4 approve×3+activate×3(hugo TTY)→**Wave 1 首輪最小 harvest 端到端**:fraser/sec adapter_config 對映補齊(DB 列、零改碼)、oai_pmh 修 `--limit`;落地 knowledge_item=fraser 2 document+oapen 61 book+sec 2 report;**全文層誠實缺口=需源專屬抓取**(FRASER 文字端點/EDGAR URL 組裝/OAPEN PDF)→plan-first 待拍板;④**A3 sha 配方 bug 鑑識**:preregister_a3 手刻 hash(12碼/無separators)≠canonical `_sha` → check「挪門柱」假警報;舊配方重算證**內容未變**;修復全走合法路徑=舊三列 approved→superseded 留檔、`_r2` 同判準重註冊(jsonb 等值✓)、hugo TTY 重簽、寫入器改用 `_sha`;⑤**開賽前置人工關卡全清**:A2 六門(凌晨已簽)+A3 `_r2` 三門+futility+九候選凍結;strict 預檢 **14/15 綠唯 E1 紅**(=audit 補救中);⚠ evaluate 嚴禁提前跑(g5:fail 即燒 gate、須另立新 gate);⑥DB dump `20260712_Fd`:本地 `~/db_dumps/` 目錄+D 碟單檔 tar 9.9GB(56GB=35 資料+21 索引;247 表完整、concordance=分區父表 16 分區皆在 TOC);import_database.sh 自動搜尋 pattern 補 `augur_pgdump_*`;⑦**下一棒(零人工)**:audit 綠→E1 重驗→strict 全綠→unfreeze GATE evaluate→開賽(run_arena_round)。

**2026-07-12 晚間增補(封存點 archive-20260712)**:①**輸出契約入憲**——用戶英文規格拍板+二度/三度堅持:靈魂 v1.7.0→**v1.8.0**(刪「不是預測絕對漲跌幅」句,勸阻雙留痕)、憲章 v1.44.0→**v1.45.0**(三產物閉集:方向機率/horizon 級準確率/E[r] 幅度級得逐股;fail-closed 升格 DB 機械閘;不受影響之鎖完整清單入條文);**O1 機械閘已落地**(migrate_direction_product_gate_ddl.py:兩產物表 gate guard trigger+expected_ret/hit_rate 欄,負向測試雙拒);②**三鏡頭擂台候選計畫**(own_threelens_interact,35 特徵直餵+9 交互對;`reports/augur_arena_threelens_candidate_plan_20260712.md` 待 T0);③**K 計畫定稿**(know-how 全通管線升級:欄位語意橋/items 統計鏈/深抓 S2-S4/排程;4-agent 實證+Opus 三視角審查 1B+9M 全吸收;`reports/augur_knowhow_e2e_upgrade_plan_20260712.md` 待 T0+R1-R6);④advisor 方向分類器誤傷修復(「未來N天」須與方向詞共現;regression 三段復綠、服務重啟);⑤refresh 驅動器 vector_export 段參數 bug 修+Qdrant 對帳追平(差=0、私有 4,653 依法擋);⑥audit 自癒 runner 續跑中(UK 段),完成後接 E1→strict→unfreeze evaluate→擂台開賽(四項開賽拍板已在手)。
