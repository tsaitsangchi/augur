...
# 鏈序:通用收割批 → quant_finance 圈域 → software_engineering 圈域 → 自我求知 → 演化迴圈(三源教材+pack+eval)
#      → 現況摘要(候選 pack 待簽清單)。flock 防重疊;各步失敗不斷鏈(獨立 log 誠實記錄);晉升永遠人閘。
# 執行指令矩陣:
...
flock -n 9 || { echo "$(date '+%F %T') 另一條鏈在跑,略過" >> "$LOG"; exit 0; }
{
  ...
  echo "── 4/5 自我求知(缺口→query) ──"
  venv/bin/python scripts/evolve_self_seek.py --seek
  echo "── 5/5 演化迴圈(三源教材→pack→eval) ──"
