...
> 🚧 **狀態**：開發中。治權已立（原則精華 v1.6.0・20 條法律；憲章 v1.8.0・catalog 橫切元資料登錄 + 官方 datasets.md 入憲驅動）；`core / ingestion / audit / features / universe` 已建並實跑，raw 全市場全史 sync 暫停、轉 catalog-driven 規劃（已產 `docs/datasets_zh.md` 全 83 表逐欄 catalog + 抓取最佳化計畫/token 預算）；`models / evaluation`（F3）未建。
...
| [`docs/原則精華_v1.6.0.md`](docs/原則精華_v1.6.0.md) | **20 條不可違反原則**（三條基石：#1 / #8 / #15） |
| [`docs/系統架構大憲章_v1.8.0.md`](docs/系統架構大憲章_v1.8.0.md) | **憲法**：三個敵人 × 管線 + 12-PHASE 維運 + 升版規則（+ 橫切 catalog 元資料登錄 + 官方 datasets.md 入憲）|
