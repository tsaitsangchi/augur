...
> **排序與級別之關係（2026-07-17 補註）**：本節之排列係依**初步判定**之嚴重度。其後經門檢與**解釋裁決第 2026-001 號**，部分項目之級別變動（如 **AUD-21 minor→MAJOR**、AUD-09 critical→MAJOR），**AUD 編號一律不重排**（編號穩定優先，重排將破壞既有引用之 ID 穩定性）。故**本節之排列順序不得逕行讀作終局嚴重度**；終局級別以**各條標題**及 **7.5（Steward 解釋裁決第 2026-001 號）**為準。
>
> ⚠️ **【更正，2026-07-17】本補註前版並列「7.2／7.5」為終局級別之權威——就 AUD-09 而言，該指引將讀者導向已被推翻之值。** **7.2 之欄位為「驗證後級別」，係門檢階段之中間值**：其 AUD-09 列（:524）載 **critical**，而 **7.5 之解釋裁決**（採選項 (A)，將「原始 vs 衍生＋緩解程度」尺度明文化）已將 AUD-09 定為 **major**，AUD-09 條標題（:236）亦載「經解釋裁決 2026-001 回 MAJOR」。故 **7.2 不得逕引為終局**——依前版指引查 7.2 者，將得與各條標題及裁決**相反**之級別。（本補註自身之「AUD-09 critical→MAJOR」敘述則屬正確；錯的是它指的地方。）複驗指令：`grep -n 'AUD-09' audits/CODE-COMPLIANCE-AUDIT-2026-07-16.md`（比對 :110／:236 標題／7.2 表列 :524／7.5 表列）。
