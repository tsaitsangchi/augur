...
| 11 | **結構補正施工**：entity registry/claim/lifecycle/屬性 as-of（AUD-04/05/06/07）、行動六元組 log（AUD-10/11）、prediction append-only 伴生表（AUD-08）——9 table＋python，全新增式 | AUD-04/05/06/07/08/10/11 | augur-code 分支 | ✅ table＋python 完成（@ 7932ba9，已推 GitHub；三審 18 issue 15 修 2 升裁決；待人類實測＋P5）。既有消費者改繫＋AUD-01 SQL 直綁消除為 follow-up Phase 1-5 |
...
1b. ~~**充任認定前置（Layer 2–4 共同）**：各層合規聲明依 `AUGUR-WM v1.0 §WM.44` 須有**逐條對應矩陣**~~ ✅ **已解（2026-07-17）**：ONT／ID／KS 三層 Annex TR **逐條完整枚舉、缺 0 條**，`§WM.44` 形式充分性成就，已隨充任認定生效（裁決 2026-003／004／005）——依據見 `IDENTITY-SPECIFICATION.md:17`、`KNOWLEDGE-SYSTEM-SPECIFICATION.md:17` 之【地位】節逐字記載。
    > **惟形式充分性 ≠ 實質正確**（交叉參照，非本檔之認定）：`§8.3` gate 硬化後測得 —— (i) **L2（ONT）之 Annex TR 標題為 h1，其 56 列矩陣從未被 gate 讀取**，故其「缺 0 條／PASS」未經機器複核；(ii) ONT 之 Annex TR 標 **[I]** 而 ID／KS 標 **[N]**，同一義務之標注不一致。二者均屬 **Steward 事項**（生效規格之編輯屬 `§8.5`／`§8.6`），本檔不作認定，見 **HANDOFF 待裁 #22**。
