...
def sync_by_date(conn, dataset, *, start=None, end=None, progress=None, snapshot_reason=None):
    ...
    snapshot_reason：**預設 None＝主路徑（daily 增量 sync）不快照、語義不變**；heal 呼叫端（reconcile.
      heal_by_date / daily_maintenance heal）透傳 'heal_by_date'/'daily_heal' → 覆寫前把被取代原值快照
      至 raw_supersede_log（AUD-02、P4.E5，與 upsert 同交易）。
    """
    if ingest.is_intraday(dataset):
        return {"dataset": dataset, "mode": "skip-intraday", "rows": 0}
        ...
        if rows:
            ...
            try:
                total += ingest.store(conn, dataset, rows, require_keys=("date",),
                                      snapshot_reason=snapshot_reason)["rows"]
                tdays += 1
