"""N5 Ollama llm_fn adapter — 本地 Ollama HTTP → advise()/審議 抽象 LLM 界面的薄接頭。

🎯 這支＝**跨包共用** LLM 通道（STRUCT：從 advisor 抽出，斷 deliberation→advisor 環）。
   make_llm_fn() 回 prompt(str)->response(str)；make_structured_llm_fn 供審議 propose。
   advisor.ollama 再匯出本模組保 BC。

執行指令矩陣（library #18；免 DB 免 API）：
  python -m augur.llm.ollama              # 印用途＋公開入口
  python -m augur.llm.ollama --selftest   # 純紅綠自測（零 IO）
"""
import json
import os
import re
...
if __name__ == "__main__":
    ...
    print("(自測:python -m augur.llm.ollama --selftest;免 DB 免 API)")
