...
def canonical_features(conn, panel_dates):
    ...
    **legacy／研究對照**；生產熱路徑預設請用 `resolve_train_feats(..., source='prodset')`。"""
...
def resolve_train_feats(conn, panel_dates, *, source: str = "prodset"):
    """訓練／熱路徑特徵解析：預設 prodset（active∩覆蓋）；canonical 僅研究對照。
    空 active（prodset）→ 拋 ProdsetEmptyError（FC-empty；禁 silent fallback）。"""
    ...
    src = (source or FEATURE_SOURCE_PRODSET).strip().lower()
    if src == FEATURE_SOURCE_CANONICAL:
        return canonical_features(conn, panel_dates)
    if src != FEATURE_SOURCE_PRODSET:
        raise ValueError(f"unknown feature-source={source!r}; use prodset|canonical")
    feats = resolve_prodset_feats(conn, panel_dates, require_nonempty=True)
    if not feats:
        # active 非空但覆蓋交集空 → 同樣 fail-closed（假矩陣）
        raise ProdsetEmptyError(
            "prodset resolve empty: active features lack panel coverage (no silent canonical fallback)"
        )
    return feats
...
def _selftest():
    ...
    chk("公開入口存在(canonical_features/run_ladder/resolve_train_feats)",
        callable(canonical_features) and callable(run_ladder) and callable(resolve_train_feats))
    ...
    chk("baseline AST 零 import augur.philosophy", bad == [])
