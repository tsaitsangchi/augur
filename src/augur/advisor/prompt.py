...
DIRECTION_SIM_HONESTY = (
    ...
    "本系統方向軸(絕對漲跌機率)六門(H20/40/82/120 + 逐日 D1/D5)已**全部統計判死**(evaluated_fail)、"
...
# lock②:方向/逐日價格題之固定誠實句(短路弱 LLM、直回;繞過幻覺、保證優雅誠實)。系統自撰常數、零編造。
DIRECTION_PATH_FIXED_RESPONSE = (
    "關於「未來逐日股價變化／絕對漲跌方向／方向準確率最高的前 N 名」——我誠實說明:\n\n"
    "本系統的**絕對漲跌方向**預測(30／60／120 日與逐日 1／5 日,共六道關卡)已全部經機械驗證**統計判死**"
    "——勝率不顯著優於「永遠猜多數方向」、且機率品質不達標。因此**沒有可信、可交易的方向或準確率**可以給你,"
    "也**無法**列出「準確率／報酬最高的前三支」及其預測股價。這不是功能缺失,是誠實:方向在這份資料上不可預測。\n\n"
    "・逐日**價格點位／路徑**永久不是本系統的預測產物。若想看「歷史波動下的可能區間」,請用**蒙地卡羅模擬情境**頁"
