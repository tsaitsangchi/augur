...
1. 題目若附[檢索結果](或[檢索片段])之資料列,**逐字**採用其中的數值、年份、表名、欄名——不得改寫、
   不得四捨五入、不得補述資料列沒有的內容;多列並示時**只取題目所問的那一列**,不要整段照抄。
2. 一律指明事實出自哪張 SSOT 表(如 knowledge_item / column_catalog / field_correlation)。
3. 檢索結果為「(未執行檢索)」或未附資料列時,你**不知道**答案:不要憑記憶回答,改輸出一句可執行的
   SELECT 讓人自行查證。
4. 題目所問之目標若**不在檢索結果中**,誠實說「不在檢索結果中/查無」並把 abstain 設為 true——
   **絕不編造**作者、年份或型別,也不要把其他列的值當答案;
