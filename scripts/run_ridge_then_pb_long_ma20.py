#!/usr/bin/env python
"""已完成八窗日 → 做多 Top10＋均線多頭＋均價差≤20% → 收盤買進（不碰 WF 鎖）。

🎯 這支在做什麼（白話）：只讀「同日 RankRidge 八窗模型已齊」的交易日。
   相對強 Top10 當池（還沒回撤也不剔除）→ SMA5>…>SMA240 且八條均價
   (最高−最低)/最低 ≤20% 才標可當進場條件，並以該日還原收盤寫入
   `ridge_then_pb_long_ma20_buy`。不覆寫 ma10／既有做多表；不拿訓模河鎖。

執行指令矩陣:
  python scripts/run_ridge_then_pb_long_ma20.py --selftest
  python scripts/run_ridge_then_pb_long_ma20.py --dry-plan
  python scripts/run_ridge_then_pb_long_ma20.py --apply --limit 1
  python scripts/run_ridge_then_pb_long_ma20.py --apply --watch
"""
...
PROGRESS = ROOT / "audits" / "RIDGE-THEN-PB-LONG-MA20-PROGRESS.json"
LOCK = Path("/tmp/augur_ridge_then_pb_long_ma20.lock")
...
def _print_long(payload: dict) -> None:
    ...
    print(
        "做多-MA20 asof=%s n_pool=%s 可當進場=%s 未過均線=%s"
        % (
            payload.get("asof"),
            pack.get("n_pool"),
            pack.get("n_entry"),
    ...
    print("── 做多｜相對強 Top k 不剔除｜SMA5>…>SMA240 且均價差≤20% ──", flush=True)
...
def run_one(asof, *, apply: bool, source: str = "pv") -> tuple[int, dict]:
    ...
    with db.connect() as conn:
        ...
        payload = ma.apply_ma_stack_payload(payload, hist, limit=LIMIT)
        payload["note"] = store.LONG_MA20_NOTE
        _print_long(payload)
        if not apply:
            return 0, payload
        closes = fetch_closes(conn, asof_ready.as_date(iso), sids)
        payload["long_close"] = store.persist_long_ma20_close_buys(conn, payload, closes)
...
def _acquire_lock():
    ...
    try:
        ...
        fd.close()
        print("✗ 另一輪做多 MA20 收盤帳在跑（%s）" % LOCK, file=sys.stderr)
        return None
    fd.write("%s\n" % Path(__file__).name)
    fd.flush()
    return fd
...
def _selftest() -> int:
    ...
    print("[ridge-then-pb-long-ma20 selftest]")
    ...
    chk("限 20%", LIMIT == 0.20)
    chk("買進表", store.TABLE_LONG_MA20_BUY == "ridge_then_pb_long_ma20_buy")
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="已完成八窗日做多均線±20% 收盤買進排程")
    ...
    try:
        ...
        while True:
            with db.connect() as conn:
                ...
                already = set() if args.force else store.processed_long_ma20_asofs(conn)
            ...
            for d in todo:
                if args.limit and n_done >= int(args.limit):
                    ...
                    return 0
                src = "pv" if d in done_pv else "dry_run"
                print("\n>>> asof=%s source=%s persist-long-ma20-close" % (d, src), flush=True)
                try:
