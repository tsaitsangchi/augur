...
#   bash scripts/backup_database.sh --run        # 跑一輪:dump→驗 toc→鏡像→驗鏡像+記帳→輪替
...
run() {
  ...
  # 冪等跳過只跳「重跑 dump」,**不跳驗鏡像**:上一輪之後鏡像可能被刪/被截斷,
  # 舊碼於此徑直接 exit 0 ⇒ 同一日重跑會在鏡像已消失時仍印 ✓(同型假綠,M-O2)
  [ -d "$dest" ] && {
    echo "✓ 今日已備份($dest)——dump 冪等跳過;鏡像仍逐次驗(冪等≠免驗)"
    mirror_sentinel || { echo "✗ 鏡像未通過可還原驗證——異地層形同不存在" >&2; exit 5; }
    exit 0
  }
