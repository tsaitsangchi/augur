...
def auth_headers(cfg, base=UA):
    """adapter_config.auth_header={"name","env"} → key 走 header 不進 URL(不落 log/provenance)。"""
    ah = (cfg or {}).get("auth_header")
    v = os.environ.get(ah.get("env", ""), "") if ah else ""
    return {**base, ah["name"]: v} if v else base
...
def adapter_generic_json(cur, src, args, dom, et):
    ...
    cfg = src[5] or {}
    d = get_json(fill(src[4], args), headers=auth_headers(cfg))
...
def adapter_oai_pmh(cur, src, args, dom, et):
    ...
    for rec in ET.fromstring(xml).iter("{http://www.openarchives.org/OAI/2.0/}record"):
        ...
        if args.limit and n >= int(args.limit):   # honor --limit(#25 最小單位;整頁不設限=首輪放量)
            break
    return n
