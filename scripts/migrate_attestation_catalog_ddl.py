...
SNAPSHOT = ("TaiwanStockInfoWithWarrant", "TaiwanStockInfoWithWarrantSummary",
            ...
            "TaiwanFutOptTickInfo")                       # 期權合約 info 維度表(code/callput/name、date=合約月未來日)——名稱含 Tick 但實為合約規格 snapshot
...
CADENCE = ("TaiwanBusinessIndicator",                      # 月頻(每月1日);滾動14日窗常空非死→#29 假綠 blocker 之低頻誠實豁免
           ...
           "TaiwanStockCapitalReductionReferencePrice")    # 減資參考價事件稀疏(678列、月間隔);2026-07-14 --audit-only 空視窗→cadence
...
INTRADAY = ("TaiwanFuturesSpreadTick",)                    # tick 串流:全欄 PK+一日數千 tick,API 重抓回不同子集→byte 逐日對帳假 EX(EX~4999);非定案 as-of、零預測用途→豁免(2026-07-14 --audit-only ③ 深追)
    ...
    with db.connect() as conn:
        with db.transaction(conn) as cur:
            ...
            cur.execute("ALTER TABLE dataset_catalog ADD COLUMN IF NOT EXISTS attestation_mode "
                        ...
                        "CHECK (attestation_mode IN ('byte','snapshot','restating','coverage','cadence','dim_only','intraday'))")
