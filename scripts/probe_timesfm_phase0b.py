#!/usr/bin/env python
"""NF-D-TIMESFM Phase 0b — TimesFMRank25 方向 hit vs naive（排序臂／預訓練）。

🎯 另書量尺:core 單股、月步 walk-forward、log(q50終/末價) 符號＝方向 vs naive。
   先決: forecast 非 NaN／finite 分數覆蓋率；預凍: mean(TimesFM hit) > mean(naive)。
   offline-local；≠升格。
守 #32b· #8· skip-sync· 禁稱可交易。

執行指令矩陣:
  python scripts/probe_timesfm_phase0b.py
  HF_HUB_OFFLINE=1 python scripts/probe_timesfm_phase0b.py --run --asof 2026-07-31 --horizon 20 --n-stocks 300
"""
...
from augur.models.timesfm_rank import TimesFMRank25
...
def _nan_coverage_gate(model, horizon: int, min_cov: float, n_trials: int = 8) -> float:
    """0a 殘差先決：真載 forecast 須有足夠 finite 分數覆蓋率。"""
    rng = np.random.RandomState(42)
    ok = 0
    for _ in range(n_trials):
        # 溫和 RW 正價（對齊 0a real selftest 精神）
        ctx = 100.0 * np.cumprod(1.0 + 0.002 * rng.randn(64))
        try:
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                s = float(model.predict_scores([ctx], horizon=horizon)[0])
            if np.isfinite(s):
                ok += 1
        except Exception:
            pass
    return ok / float(n_trials)
...
def main(argv=None):
    ...
    ap.add_argument("--min-finite-cov", type=float, default=0.5)
    ...
    print(
        f"預凍門檻: mean(TimesFM hit) > mean(naive hit)；offline-local；no-promote；"
        f"n_stocks={args.n_stocks} h={args.horizon} asof={args.asof} "
        f"max_folds={args.max_folds} min_finite_cov={args.min_finite_cov}",
        flush=True,
    )
    model = TimesFMRank25(horizon=args.horizon, local_files_only=True).fit()

    # 先決：權重可載＋非 NaN 覆蓋率
    try:
        cov = _nan_coverage_gate(model, args.horizon, args.min_finite_cov)
        print(f"NaN 覆蓋率 gate：finite_cov={cov:.3f}（門≥{args.min_finite_cov}）", flush=True)
        if cov < args.min_finite_cov:
            print(
                "✗ forecast finite 覆蓋不足（誠實 STOP／SKIP；"
                "對齊 0a CPU NaN 殘差；≠塗綠 stub）",
                flush=True,
            )
            return 2
        print("模型預熱＋覆蓋率：OK", flush=True)
    except Exception as e:
        print(f"✗ 模型預熱／覆蓋率失敗（誠實 STOP）: {e}", flush=True)
        return 2
    ...
    with db.connect() as conn:
        ...
        for j, sid in enumerate(stocks):
            ...
            if (j + 1) % 10 == 0 or j == 0:
                print(
                    f"  [{j+1}/{len(stocks)}] {sid} timesfm={mh:.3f} naive={nh:.3f} folds={len(m_h)}",
                    flush=True,
                )
    ...
    if m_mean > n_mean:
        print("閘：有證據（mean TimesFM > naive）→ 仍 STOP promote（預凍）", flush=True)
        return 0
    print("閘：無證據（mean TimesFM ≤ naive）→ STOP", flush=True)
    return 0
