...
def render_pack(pack: Mapping[str, Any]) -> list[str]:
    ...
    out.append("   # 先乾跑看引擎裁決（唯讀）:")
    out.append(f"   venv/bin/python scripts/apply_evolution_promotions.py --run-id {rid} --dry-run")
    out.append("   # 逐顆核對上列後，由 hugo 親跑開閘（I5 落帳留 gate_ref）:")
    out.append(f"   venv/bin/python scripts/run_evolution_iteration.py --step I5 --allow-apply --gate-ref {HUMAN_GATE_REF}")
    out.append("   ⚠ 顆粒誠實註記: I5／apply_evolution_promotions 現行 CLI 無逐 queue_id 旗標——一次 APPLY")
    out.append("     消費該 run 全部 pending_auto（含 demote FAIL_SIGN 自動通道列）；「逐顆」之裁法＝先核")
    out.append("     「可裁清單」與「pending_auto 全集」一致；有不欲 APPLY 之列即停手先問（#6），勿旁路。")
    out.append("")
