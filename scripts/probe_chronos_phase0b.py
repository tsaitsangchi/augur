#!/usr/bin/env python
"""NF-D-CHRONOS Phase 0b — ChronosRankBolt 方向 hit vs naive（排序臂／預訓練）。

🎯 另書量尺:core 單股、月步 walk-forward、log(q50終/末價) 符號＝方向 vs naive。
   預凍: mean(Chronos hit) > mean(naive)。offline-local；≠升格。
守 #32b· #8· skip-sync· 禁稱可交易。

執行指令矩陣:
  python scripts/probe_chronos_phase0b.py
  HF_HUB_OFFLINE=1 python scripts/probe_chronos_phase0b.py --run --asof 2026-07-31 --horizon 20 --n-stocks 300
"""
...
from augur.models.chronos_rank import ChronosRankBolt
...
def main(argv=None):
    ...
    print(
        f"預凍門檻: mean(Chronos hit) > mean(naive hit)；offline-local；no-promote；"
        f"n_stocks={args.n_stocks} h={args.horizon} asof={args.asof} "
        f"max_folds={args.max_folds}",
        flush=True,
    )
    model = ChronosRankBolt(
        horizon=args.horizon, local_files_only=True
    ).fit()
    try:
    ...
    with db.connect() as conn:
        ...
        for j, sid in enumerate(stocks):
            ...
            if (j + 1) % 10 == 0 or j == 0:
                print(
                    f"  [{j+1}/{len(stocks)}] {sid} chronos={mh:.3f} naive={nh:.3f} folds={len(m_h)}",
                    flush=True,
                )
    ...
    if m_mean > n_mean:
        print("閘：有證據（mean Chronos > naive）→ 仍 STOP promote（預凍）", flush=True)
        return 0
    print("閘：無證據（mean Chronos ≤ naive）→ STOP", flush=True)
    return 0
