...
**Qdrant serving 已上線（hugo 2026-07-14 拍板「一定要上 Qdrant」）**：`augur-qdrant.service`（systemd user、開機自啟）、storage=`~/qdrant_augur`（augur 專屬、不共用 ttai）、二進位暫用 `~/project/ttai/.qdrant_server/qdrant`（native 85MB）。sentence_items public en 55,861 向量 synced 差=0；config `sentence_items=qdrant_server` active；shadow eval **0.972 PASS**。pgvector 仍 SSOT、Qdrant 可拋棄隨時從 PG 重建。
