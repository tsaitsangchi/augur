...
description: "抓取唯一資料本質排除=日(#4)；⚠2026-07-17更正:OUT_OF_UNIT『06-16證偽→可抓暫緩』已被06-23/24 probe再翻回『治權級排除』(ingest.py:32-46現載3表排除、BACKFILL_DEFERRED=空集);本檔內文為鐘擺中間位、方向相反於現行code"
metadata: 
  node_type: memory
  type: feedback
...
> ⚠️ **2026-07-17 更正**:下段「證偽→BACKFILL_DEFERRED」是鐘擺中間位。**06-23/24 probe 再翻回治權級排除**——`ingest.py:32-46` 現載 `OUT_OF_UNIT` 3 表(明文「規模物理界限排除、非暫緩」)、`BACKFILL_DEFERRED = frozenset()` 空集。「dedicated endpoint 可抓」capability 屬實仍在,但「該不該進 auto-backfill」的結論已定為**排除**、非「暫緩待決」。
