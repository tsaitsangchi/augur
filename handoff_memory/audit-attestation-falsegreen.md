...
**(a) E1 gate 讀持久化 verdict(2026-07-14 hugo 拍板;E1 前置揭露)**:E1 原 check_cmd=同步跑 reconcile_audit.py→(1)39分>harness 600s timeout (2)窗/heal 與正典 daily_maintenance 分歧(reconcile_audit 硬編 UNSETTLED_BUFFER=2+無 heal+30天窗→UKStockPrice 外盤 settling/除權息 VM8978;daily 用 catalog finalize_lag+heal→VM0)。修=正典驅動(daily_maintenance --audit-only --heal)跑完寫 `attestation_result` 表(schema INFRA_DDL 冪等建),E1 改 sql check 讀「最近 daily_maintenance PASS 且 run_at≤2日」(freshness 防陳舊綠)。run(慢 async)與 gate 檢查(快讀表)解耦。**正典綠=daily_maintenance;reconcile_audit 退為輔/棄用於 gate**。