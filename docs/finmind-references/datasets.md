<!-- ════════════════════════════════════════════════════════════════
augur 治權參考檔 — 入憲 2026-06-15（憲章 v1.8.0、catalog 橫切層權威輸入源）
來源：FinMind 官方 https://github.com/FinMind/FinMind（.claude/commands/finmind-references/datasets.md）
性質：FinMind 官方維護之全 dataset 抓法元資料（tier / params / columns / 分類）= augur 抓取「怎麼抓」之權威 SSOT。
用途：catalog builder（src/augur/catalog）解析本檔驅動 tier / intraday / sponsor / single-day，取代 code 硬編白名單（守 #3/#18 無硬編）。
治權位階：官方權威參考（API 行為之文檔化），與 live API 探測並列為 #18「依 API 探測」之依據。Layer 7 登錄（RULING-2026-002）；與 datasets_zh 共用合規聲明 docs/compliance/CS-datasets_zh.md（AUGUR-MC v1.6）。
維護：官方原文、可刷新（curl 覆蓋後保留本 header）；augur 不改官方內容，augur 策展（中文/髒值/OUT_OF_UNIT）另住 catalog 表 + datasets_zh.md。
════════════════════════════════════════════════════════════════ -->
