# 模擬方法與自進化元件專章 v1.0——普遍晉升路徑之本軸門檻

> **生效**：2026-07-31，經 `governance_proposal` **gp_86c8063fc688**——Steward（hugo）
> 於 TTY 親簽 approve（note：「axis registry 性質認定: patch」）、enact 落地。
> **登錄位置**：`docs/系統架構大憲章`第三部之下位專章（本檔＝條文 SSOT，憲章置指標不複述）。
> **選定存卷**：①終審＝統計級校準檢定（非實效級 #14）；②人簽＝偵測級＋三項最低補強；
> ③axis 登錄＝registry 表（性質認定＝patch，Steward 以 approve --note 載明）。
> **繕打**：claude（`AUGUR-MC v1.6 §8.1`／`RULING-2026-028`：繕打非參與）；
> 議決全程之提案凍結、TTY 親簽、enact 記錄住 `governance_proposal`。
> 提案版原文（含當時待裁格式）：`reports/augur_sim_evolution_chapter_final_20260731.md`。
---
