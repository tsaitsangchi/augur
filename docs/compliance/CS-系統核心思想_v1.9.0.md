...
> **CS.4 跨層標注（實況；v1.9.0 續批 2026-07-30）**：本檔主體登錄 **Layer 1**，惟正文下列節具**跨層落點**，依 `AUGUR-MC v1.6 §0.5` 逐節標注——「**它不做的事**」之下單禁令（不替使用者下單、不動錢、自動下單＝禁）→ **L6**（承 `§A.53`）；「**管線**」之各層職責不越界（驗證不讀訓練產物）與「單一引用源」→ **L7／L5**（承 `§A.51`，機器強制屬 DEFER）；「**為誰**」之 RBAC 讀取範圍宣告 → **L6／L7**。（原聲明未載跨層落點，與正文不符——正文確含 L6 行動禁令與 L7 架構不變式。）
