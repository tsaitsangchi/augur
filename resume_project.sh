...
echo "═══ 接續前置完成 ═══"
echo "後續(依需要):"
echo "  排程重建(2026-07-26 補齊;cron/systemd 皆機器本地、不隨 git):"
echo "    bash install_services.sh          # systemd 服務棧+timers+drop-in(LLM 單槽鎖/時間平移)"
echo "    bash install_cron.sh              # 先唯讀看 diff;確認後 --apply(舊機有既存條目則 --migrate)"
echo "  讀 HANDOFF.md §3(排程/不在 git 清單)與 §4 進度、reports/augur_construction_understanding_20260710.md 建構全貌。"
