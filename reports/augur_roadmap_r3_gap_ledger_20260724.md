...
| ID | layer / clause | obligation_summary | claimed_landing | actual_evidence | gap_class | next |
|---|---|---|---|---|---|---|
| G-PV-1 | 隔離／COMMENT | `prediction_values` 禁回讀當特徵 | AST PV-α（PREDICT_CONSUMERS） | **R5-S12 2026-07-24**：`import_isolation.PRODUCT_LITERALS`＝`prediction_values`／`prediction_probability`；`check_isolation()` 綠；COMMENT 改誠實文（AST 閘；GRANT SELECT 仍准 writer、β 未做）。`python -m augur.audit.import_isolation` exit 0；pytest 紅測。見 `audits/ROADMAP-R5-S12-CLOSED-20260724.md` | **none** | 維持；β REVOKE SELECT 另授權（須證明 writer 零自讀） |
