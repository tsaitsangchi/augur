...
| # | 決定事項 | 選項 | 裁示 |
|---|---|---|---|
| 1 | 讀法（§3／§8） | 甲／乙／丙＋§8.4 聲請／其他 | ＿＿ |
| 2 | B 類 15 檔歸屬（§9 Q1） | 入射程／除外／逐檔另裁 | ＿＿ |
| 3 | DDL 形制（§4）與 append-only 是否上 trigger | 照案／修改 | ＿＿ |
| 4 | Annex F 六條採認方式（§9 Q4） | 本案簽核即附卷／另立 RULING | ＿＿ |
| 5 | G0 拍板日與 M3 硬里程碑（§6） | 照案（08-16／10-05）／調整 | ＿＿ |
| 6 | lint 閘可否於 G0 前先行上線止血（僅凍 baseline、不含白名單） | 准／不准 | ＿＿ |
