# 全文落地與逐字理解計畫 v1.6(text corpus → verbatim understanding →「博學的我」)
...
CREATE TABLE IF NOT EXISTS knowledge_item_text (
  ...
  license    varchar(64) NOT NULL CHECK (license IN ('public_domain','cc-by','cc-by-sa','cc0')),  -- v1.5:憲章 v1.20.0 雙軌拍板,CC 白名單納入(NC/ND/未明停 metadata)
...
### T-1 `audit_work_attribution.py`(v1.6 新增,**T3/T4/T5 之硬先決**)
- 動因(雙重驗證實證):既有 corpus 已有 Gutenberg 同名誤配污染——**85%(65,033/76,795)chunks 出自未稽核的 538 部自動遍歷集**,抽樣即中新誤配(work 364 建築誌掛 Henry George、106 chunks 已入 pgvector 可被 advisor 引用)。
- 職責:對全部 work(自動遍歷來源優先)跑**歸屬稽核**——作者姓名全等比對(gutendex 回查)+ 著作年代 vs 作者生卒粗檢;不符者標 `philosophy_work.review_flag=true`(新欄,DDL 入 T0)。
- **級聯清理**:review_flag work 之 work_text/chunk/embedding **一併隔離**(chunk 標記或刪除重建;不留污染向量)。
- 驗收 SQL:`SELECT count(*) FROM philosophy_work WHERE review_flag IS NULL`=0(全稽核)+ 抽樣 mismatch=0;**T3/T4/T5/L5 一律 `WHERE NOT review_flag`**(builder 排除機制)。
- 矩陣:`python scripts/audit_work_attribution.py [--source-note 自動遍歷] [--fix]`。
...
- 職責:knowledge_item(有 DOI、無 item_text)→ Unpaywall 查 `best_oa_location` → **license ∈ {public_domain, cc-by, cc-by-sa, cc0}**(憲章 v1.20.0 雙軌拍板;**NC/ND/版權未明一律停 metadata**,license 值逐字存入附來源)且 content-type ∈ {text/html, text/plain} → HTML 剝標籤(規則)→ item_text。**PDF 一律跳過記 log**。
...
- 前置:**6 源**辭書/註疏全文先經既有 fetch 工具入 work+work_text(v1.6 計數同步 CLI)(來源:維基文庫〔說文/康熙/王弼注/十三經注疏〕、Gutenberg Webster 1913 / Roget——**確切 ebook ID 執行時經 gutendex 實測確認(#25),計畫不寫死記憶值**;康熙字典維基文庫完整性同樣執行時實測)。
...
### 執行相依圖
```
T-1(歸屬稽核+級聯清理,硬先決) → T0 → T2(辭書先行) → T3(--scope philosophy) → T4(同 scope,實測效能)
...
## 五、量估
T2 ~1-2 天背景(6 源辭書/註疏);T3+T4 哲學期 ~數小時;全 corpus concordance 數億列=分區+分期(p0 實測後定節奏);T1 隨 harvest;嵌入 e5-small 6.7 塊/s。
...
## 八、驗收判準
1. lexicon:「道」→ 說文+康熙+王弼注、「仁」→ **十三經注疏**(v1.6 補抽驗例)、"value"→ Webster,逐字可溯 locator
...
## 十一、v1.5 修訂(2026-07-02):CC 白名單依憲章 v1.20.0 拍板恢復(item_text CHECK 含 cc-by/cc-by-sa/cc0;哲學原典 work_text 限公版不動)。

## 十二、v1.6 修訂(雙重驗證 fix_gaps,2026-07-02)
[med·最重] **T-1 歸屬稽核閘新增**:corpus 污染(85% chunks 未稽核、work 364 實證誤配已入向量庫)——姓名+年代稽核、review_flag 欄、chunk/embedding 級聯清理、builder WHERE NOT review_flag、驗收 SQL;列 T3/T4/T5 硬先決|[low] 辭書計數 5→6 源同步、驗收 1 補十三經注疏抽驗例
