# 全球 know-how 落地計畫 v1.2(harvest:registry → PostgreSQL 正式表)
...
1. **載排程(兩型,§八缺陷4)**:
   - **(a) 查詢型**(template 含 `{query}`):
     ...
     JOIN knowledge_source s ON s.enabled AND (s.query_template LIKE '%{query}%' OR s.query_template LIKE '%{query_raw}%')
       ...
       AND s.entity_type = 'work'   -- v1.2 entity 閘:topic 詞不配 thinker 型源(否則 9,412 組合把概念實體灌入策展 thinker 表)
     ...
     LIMIT {batch}
     ```
   - **(b) 單跑型**(enabled 且 template 非空且**同時不含 `{query}` 與 `{query_raw}`**,v1.2 修:unpaywall_doi/wikidata_people 用 {query_raw} 屬查詢驅動、非單跑):每源獨立跑一次;**limit 依 #18 探測**——首跑 500,若回滿 500 → 記 log 並翻頁/提高再跑(500=初值非寫死上限,v1.2);log 以 `query_id=0` 記錄。
...
6. **實作註記(v1.2)**:排程 SQL 一律 psycopg **參數化執行**(`%s`),`{batch}`/`{query}` 為文件示意——不得用 `.format()`(SQL 內含 `%`/`{query}` 字面必炸);harvest 啟動時印 **domain_map 未配對域統計**(INNER JOIN 靜默剔除偵測)。
7. **指令矩陣**:
   ```
...
- 排程空間(v1.2 依五鏡 DB 實算更正):entity 閘後查詢型 work 源 × 4,706 詞 ≈ **數萬-8 萬組合級**(原 28-30k 低估近 3 倍;**執行時以排程 SQL count 實算印出,計畫值=快照非承諾**)+ 單跑型(實算 28 源)
