# 全球 know-how 落地計畫 v1.1(harvest:registry → PostgreSQL 正式表)
...
CREATE UNIQUE INDEX uq_item_title ON knowledge_item (entity_type, md5(title), COALESCE(year,0));
...
```
種子=`SELECT DISTINCT domain FROM knowledge_query`(恆等列預設),再套**初始覆寫對映(明文列舉)**:
`materials_science→energy_materials`·`energy→energy_materials`·`economics_econometrics_and_finance→finance_mgmt`·`business_management_and_accounting→business_mgmt`·`decision_sciences→organization_mgmt`·`chemistry→chemistry`·`chemical_engineering→chemistry`·`physics_and_astronomy→physics`·`biochemistry_genetics_and_molecular_biology→biology`·`engineering→electronics`(電子相近域)——其餘 16 field 保留原 slug 為新域名。**新對映=INSERT/UPDATE,零 code**。實際 slug 以 `SELECT DISTINCT` 結果為準(公式 SSOT 在 expansion script,#25 執行時核)。

### 4. 既有表補欄(③′/①′)
```sql
ALTER TABLE knowledge_staging ADD COLUMN query_id int;        -- acquire --query-id 蓋章(§八缺陷2)
ALTER TABLE knowledge_query   ADD COLUMN taxonomy_id int REFERENCES knowledge_taxonomy(tax_id);
...
## 三、落地過程作法(harvest_knowledge.py 逐步)
...
1. **載排程(兩型,§八缺陷4)**:
   - **(a) 查詢型**(template 含 `{query}`):
     ...
     JOIN knowledge_source s ON s.enabled AND s.query_template LIKE '%{query}%'
     ...
     LIMIT {batch}
     ```
   - **(b) 單跑型**(enabled 且 template 非空且無 `{query}`,如 dbpedia 獎項類):每源獨立跑一次、**明示 `--limit 500`**(獎項得主數百人,預設 20 會假完整、違 #15),log 以 `query_id=0` 記錄。
...
3. **輪末 promote**:`thinker` → `work` → `item`(人物先於著作;item 帶 taxonomy_id)。
...
- 排程空間:查詢型 ~4,700 詞 × 相容源(general 6 + 域源)≈ **28,000-30,000 組合** + 單跑型 ~15 源
- 每組合 ~3-4s(subprocess 啟動 ~1s + API + sleep)→ `--batch 300` ≈ **20-25 分/輪**
- `--rounds 4 --max-minutes 120` = 一次背景 ≈ 2h 吃 ~1,200 組合 → 全量 ~25 次背景啟動(nightly 數週)
- 優先序:手動 190 詞 → 高 works_count topic → 長尾;隨時停、永遠續;AI 不自掛喚醒鏈(#28)

## 五、紀律

- **#25 首輪驗證**:`--batch 10 --rounds 1` 全鏈通(staging→item、lineage 四鏈)才放常規批
- **#17**:限速矩陣+熔斷;429/503 記 error(attempts++)不重試風暴;attempts≥2 除役出排程(手動 reset 可復)
...
1. 首輪 `--batch 10`:全鏈通;**lineage 四鏈實證** `item → staging(query_id) → query(taxonomy_id) → taxonomy` + `source_key` 皆非空
2. 中斷重跑 resume 實證(已跑組合 skip;error attempts 上限生效)
3. 去重實證(同 DOI 重抓不重複)
4. 單跑型來源(dbpedia 獎項)獨立執行+log(query_id=0)實證
5. domain_map 對映生效(taxonomy 詞配到域專源)實證一例(materials 詞 × osti)
...
2. `knowledge_domain_map` 種子=`SELECT DISTINCT domain FROM knowledge_query` 資料驅動(恆等列預設、含手動域),特定對映以 UPDATE 補——slug 公式 SSOT 在 expansion script,本計畫不重複定義。
3. 單跑型來源明示 `--limit 500`(防獎項名單假完整)。
