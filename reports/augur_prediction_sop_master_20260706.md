...
| **b** ✅經濟閉環 | survivorship：`core_gate` 全史齊＝倖存/incumbency 名單。**✅ 2026-07-08 經濟重跑(3 鏡 CONFIRM、`survivorship_economic_verdict_20260708.md`):拆兩效應——①經典下市 survivorship 邊際 ≈0(+0.0023 Sharpe、16 clearing 事件 0 落 top-decile)=下市偏誤實證解決;②完整度閘 incumbency(全史齊 vs 當下可算)= −16.5%(淨 Sharpe 1.20→1.00),屬宇宙定義決策(point-in-time 無 look-ahead、但偏連續在世老股)。IC −8.5% 同理主為②非①** | `universe/core_gate.py:202 build_universe_asof`(全史齊);`scripts/survivorship_economic_verdict.py`(可複現) | **①下市已閉環(≈0)**;②兩宇宙並存標註(用戶拍板 2026-07-08):headline 1.20(全史齊穩定核心)/ 1.00(當下可算廣宇宙、更誠實可部署);清算 label==生產(0 diff、#8 乾淨) | ✅ C/D 併 |
