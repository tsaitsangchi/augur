...
## 八、每條主張之證偽條件（**本節為 §九 自我利益揭露之對價**）

> `AUGUR-L6 v1.2` L6.18(c)：self-reported **不得單獨**作為依據。
> ⇒ **揭露是必要不是充分**。本節把每條建議附上「什麼觀察會證明它錯」，
> 把「相信起草者的判斷」換成「這裡有檢驗方法」。

| # | 主張 | **證偽條件（觀察到即代表本計畫錯）** | 何時可檢 |
|---|---|---|---|
| T1 | **核心**：先修量尺、再談產出（W4 優先於 W5） | 量尺修完後，**每一項判定之結果完全沒變**（該綠的還綠、該紅的還紅）⇒ 量尺工作是白工，排序錯誤 | 修 LAIEVO 尺前先記下現有各臂 verdict，修完比對 |
| T2 | W4 應先於 W5（sim） | sim 軸在**未做任何量尺工作**之情況下即產出可評估、可複現之輸出 ⇒ 排序錯誤 | sim W1 跑通時 |
| T3 | G-SIGN 應入 `GATE_IDS` 而非 APPLY 腳本 | 加入後出現「閘判 PASS 但 prodset 仍進了未過符號檢查之成員」⇒ 住所選錯，該回頭放 APPLY 端 | W3 落地後第一次 APPLY |
| T4 | 67 筆 `pending_auto` 重評之代價可接受 | 重評後**超過半數**由 pending_auto 掉成 rejected_gate ⇒ 加閘之破壞性被低估 | W3 後第一輪 |
| T5 | 三軸 kill switch 之接線有實效 | 設 halt 後仍有任一軸開輪或取 heavy slot ⇒ 接線無效（今日只驗到 rc 與絆線，**未於真實 halt 下跑過一輪**） | 任一次真實 halt 演練 |
| T6 | 符號尺落帳後 (b) 可機械查證 | 兩顆現役跑完 `--record` 後，週報 (b) 仍印「無紀錄」⇒ 讀寫口徑不一致 | W2-‖A 後立即 |
| T7 | 「本日三修拆掉了排定之假綠」 | 下週日 09:00 週報若仍印「(b) 達成」而 `feature_sign_check` 無 PASS 列 ⇒ 修正無效 | **2026-08-02 09:00**（首次自然到期之檢驗） |

**T7 是唯一不需任何人動手就會自己揭曉的**——它會在 08-02 早上自動證明或推翻今日之工作。
