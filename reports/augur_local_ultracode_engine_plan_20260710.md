# augur 本地審議引擎(Local Ultracode Engine)計畫 v1.3(2026-07-10;逐鏡整合定稿:四鏡 blocker+major 修法整合入正文+發現表 57 項留痕)
...
3. **多數決**:N 個獨立懷疑者(各配一失效模式鏡)投票,依**三級殺權**裁決(語意 SSOT=§2.1.2:oracle 反證單票即殺/純意見全票反駁才 discard/多數非全票僅降序)——壓單次隨機錯。
...
#### 2.1.2 模式 2:對抗驗證(N 懷疑者;殺權三級制=全計畫多數決語意唯一 SSOT)

- **機制(三級制,本條寫死;§3.2 status 註解、§3.5 `vote_kill_rule` 種子、§4.2.5 `apply_kill_rule` 一律引用本條、不得另立語意——E-21 消歧)**:N 個懷疑者(`skeptic_n=3`,住 engine_config)、prompt 偏向「反駁」;殺權分三級——
  - **(a) 證據級,單票即殺**:反駁附錨點且其指定 oracle 跑出 `refuted` 確定性 verdict → claim=`refuted`(此非投票、是機械裁決,一票夠);
  - **(b) 全票級,可復活之 discard**:純意見反駁(無 oracle 可裁)只累計票數(落 `deliberation_vote`,§3.3.8);達 `vote_kill_rule`(種子=`{min_votes:3, refute_ratio:1.0}`,全票才殺——殺錯成本>省時故保守)→ claim 轉 `discarded`(triage 資源分配、非真理裁決;列仍在帳、可人工復活);
  - **(c) 多數非全票,僅降序**:只降低該 claim 之送驗/escalation 優先序(排序訊號),照常送驗,不 discard。
  三級皆**絕不產生 confirmed**(升 confirmed 唯 `verifiers.verify_claim`);quorum/ratio 判準魔數唯一住所=`deliberation_engine_config.vote_kill_rule`。
...
- **與 Claude 版差異**:Claude 端 N 懷疑者=N 個獨立智力;本地 N 樣本出自**同一底模**(qwen3),錯誤相關、會「自信地一致錯」(前身 plan:16-20)——故 LLM 票在本地**只是 triage 訊號、不是裁決**(故殺保守到全票),裁決權全部移交確定性 oracle。此設計直接回應前身 §10.2「qwen 當 judge 品質乘零」的悲觀:不讓它當 judge,讓它當「會指路的探測器」。
...
| `pytest`(新,P4) | `subprocess` argv 直呼 `python -m pytest <nodeid> -x -q`(benchmark 模式 cwd=scratch worktree,§2.2.4-1) | 行為宣稱(測試過/不過) | **執行級(獨立約束)** |
...
**安全模型收口(E-51,與 §5.6 歸一)**:①「單語句」=verifiers.py 之**禁分號 regex 工具層約束**——本專案 DB 層為 psycopg2(`core/db.py` 實查),其 `execute` 走 simple query protocol、**無**「單 execute 拒多語句」之協定保證,任何此類宣稱作廢;裁決性防線恆為**權限層**(角色零寫入 GRANT+唯讀交易),regex 僅輔助。②本角色 `GRANT SELECT ON ALL TABLES` 含 `deliberation_benchmark_case.ground_truth`/`advisor_distill_question.expected` 可及,**僅供非 benchmark 運行**;benchmark 模式一律改連 `deliberation_bench_ro` 顯式白名單角色——**benchmark 防作弊權限 SSOT=§5.6**,harness 啟動時斷言連線角色,本節依此讀。
...
1. **只跑 repo 已 tracked 的既有測試**:node id 先過 regex `^tests/[\w/]+\.py(::[\w\[\]-]+)*$`,再以 `git ls-files` 驗證該檔受版控——LLM 不得提供測試碼內容,只能指定 node id;新增測試檔供 oracle 跑=決策層拍板(#26)。
   **diff 施用沙箱(E-26 補定;seeded-bug 基準之唯一施力機制)**:tracked 測試若直接跑在真 repo,測的是未變異碼、永遠捕不到 payload 內的植入 bug——故 benchmark 模式下 pytest 一律跑在 **scratch worktree**:`git worktree add <scratch> <case 凍結 SHA>` → `git apply` 該 case payload diff(apply 失敗=undecidable)→ cwd=`<scratch>` 跑白名單 node id → 成敗一律 `git worktree remove --force` 銷毀(teardown 進 finally;單例互斥鎖防 worktree 堆積)。變的只是「被測樹」,**測試碼白名單不變**(仍 tracked-only、LLM 只能指定 node id);舊稿「scratch worktree 內由模板生成測試」一案**廢除**(與本條互斥,依 tracked-only 收斂)。誠實風險界:git apply 的 diff 即測例植入碼,於 worktree 內執行=執行機械變異產物(origin=mutation_tool、gen_seed 可溯,§3.3.11)——本機制與 `pytest_enabled` 同閘,**整段屬 P4/D4 安全拍板範圍**。
2. **環境剝離**:subprocess env 最小化,剝除 `FINMIND_TOKEN` 等 API 金鑰與 DB 主憑證——防測試碼經 env 打外部 API 放量(#24)或觸 DB 寫入;DB 相關測試只給 `deliberation_ro` 憑證。**scratch worktree 無 `.env`**(untracked,`git worktree add`/`git apply` 皆不帶入)——屬設計而非缺陷:依賴 `.env` 之測試一律走第 6 條 env_error 路徑,不補檔、不注入秘密。
...
6. **verdict 三分:pass/fail/env_error(E-54/E-48 假陰性防)**:環境剝離+worktree 無 `.env` 使依賴 DB/env 之測試必然環境性失敗——此類失敗**不是行為反證**。收集階段錯誤(collection error)、權限/連線類例外、`ModuleNotFoundError` 等一律判 `env_error` → verdict=`undecidable`+escalate,**不得記 refuted**;唯「收集成功且斷言失敗」才是 fail=refuted。P4 前置:以剝離環境對 tracked 測試全量跑一次、分類落表為「剝離環境可跑白名單」,oracle 只接白名單內 node id。
7. **承重誠實:pytest 非 GATE 承重件(E-54;tests/ 2026-07-10 實查)**:tests/ 僅 16 個 `test_*.py`,集中 advisor/guard/ingestion/generic_schema/knowledge/models/evaluation,**`features`/`universe` 零對應測試檔**——seeded_bug 自 `src/augur/` 真實檔取樣,多數變異落在無測試覆蓋的檔,pytest 命中率結構性趨零。故明定:**seeded_bug 之偵測與計分主力=§5.4 機械比對(臂輸出 findings vs `ground_truth` 植入清單),pytest 僅為 engine 臂之選配輔證,G1 成敗不繫於 pytest**(`pytest_enabled=false` 種子即此態,關閉不影響 GATE)。P4 若啟用:語料生成時以 coverage map 機械標 `pytest_reachable`(=該 case diff 觸及模組 ∩ tracked 測試檔非空;旗標住 `deliberation_benchmark_case.payload` JSONB,零新欄),報告按旗標分桶呈現;node id 白名單同步收窄=`git ls-files tests/` ∩ 該 diff 觸及模組之對應測試。
...
> **本節裁決修訂(E-6/E-9 整合)**:節名保留歷史命名,惟**預設運行態=單模 qwen3:8b**——`fast`/`deep`/`deep_think` 三檔位同指 qwen3:8b 權重,僅以 think 與 options 區分;「fast→qwen3:4b」自預設**降級為 #27 受控實驗**,過雙閘(§2.3.3)才提拔。依據=VRAM 前提實測比舊稿更差:idle(`/api/ps` 回空、零模型駐留)時 GPU 已被佔 942MiB(WSL 宿主保留,nvidia-smi 實測)→ 可用 ≈3.1GB;qwen3:4b Q4 權重 ~2.5-2.6GB+KV cache+compute buffer@4096 ctx →「4b 全 GPU」屬邊緣;且 8b 帶 keep_alive 之交錯請求下 Ollama 可能**部分 offload 而非整模逐出**——probe 啟動時單測通過(size_vram==size)**不代表運行期成立,不得假設**。

#### 2.3.1 路由機制(DDL 住 §3.3.1/§3.3.3,本節零 DDL)

舊稿之 `deliberation_model_route` 表**廢除**(與 `deliberation_model_profile` 同 role 雙軌,對抗審查殺):檔位=`deliberation_model_profile`(tier→model_tag/think/options/timeout,**未實測禁啟用 CHECK**);步驟→檔位之路由=`deliberation_lens.model_tier`(每 LLM 步驟必經一 lens,lens 綁 tier);步驟級取樣覆蓋=`lens.options_override`(JSONB,merge 順序=profile.options ⊕ lens.options_override ⊕ 逐 call seed)。`llmroute.py::resolve(lens) -> (model_tag, think, options, timeout)`;model 為 Ollama per-request body 欄位(實證)——**model-agnostic 天然成立,換模=UPDATE model_profile 一列,零改碼**(需求 b;單模⇄雙模切換即此機制,引擎零分支)。取樣參數必顯式覆蓋:qwen3:8b Modelfile 預設 `temperature=0.6/top_k=20/top_p=0.95`(實測 `/api/show`),裁決/抽取步不覆蓋即不可重現;懷疑者多樣性用 seed=確定性導出 `hash(session_id, step_key) % 2^31`,記入帳本可重放。

#### 2.3.2 檔位設計依據與排程(全部溯源實測)

| 實測事實 | 路由含意(單模 8b 預設) |
|---|---|
| idle GPU 已佔 942MiB → 可用 ≈3.1GB;8b 僅 39% 層在 VRAM(2.31/5.96GB)→ ~7 tok/s | 8b=全步驟預設模;成本預算按單流 ~7 tok/s 編 |
| think:true 39.7s vs think:false 5.5s(~7x) | think 只開 deep_think 三步(裁決/合成/critic);fast/deep 步 think:false |
| 冷載 9.24s/次(load_duration 實測) | **單模預設=零換檔稅**(think 開關不換權重);換檔稅僅雙模實驗態存在,紀律見下 |
| `-np 1` 序列化;`/proc/337/environ` 無 OLLAMA_NUM_PARALLEL 等調校 | 路由不假設並行;`OLLAMA_NUM_PARALLEL>1` 屬 #27 逐級逼近實驗(KV cache 擠壓 4GB VRAM 可能更慢),非預設 |
| num_ctx 載入值 4096(模型上限 40960) | profile.options 帶 `num_ctx`;調大之 VRAM 代價未實測,保守 4096 起步,prompt 組裝端強制 token 預算 |
| qwen3:4b 已安裝且 MVP 實戰(`/api/tags` 兩模皆列;MVP 基準 12 列以 4b 跑) | 4b 可用性已證,但**不作預設 fast**;提拔=#27 雙閘(§2.3.3) |

**換檔紀律(修訂舊稿自相矛盾:「同 phase 內不交替、換檔≤每 phase 一次」vs §2.1.9/§4.3.4 輪內 draft(fast)→refine(deep) 逐輪交替=每輪 ≥2 次×~9s 稅)**:
- **單模預設**:fast/deep 同權重,輪內交替=只換 think/options 模板,零載入稅——矛盾在單模態下自然消失;
- **雙模實驗態**(僅 #27 提拔後):(a) tier 切換時對前一模顯式送 `keep_alive:0` 卸載(修 §3.5 `keep_alive="30m"` 種子與「按 tier 分批逐出」互斥——30m 僅適用同模連跑,換檔瞬間顯式歸零);(b) 排程**跨 round 攢批同 tier**——攢滿一批 fast 步一次換 deep,禁逐輪交替;(c) 每 call `load_duration_ms` 落帳本(§3.3.7 既有欄),>`swap_thrash_ms`(config,種子 1000)計一次換檔顛簸,swap 次數與總稅入 run report——換檔成本以帳本實測結算,不以 9.24s 單點外推(#9)。

#### 2.3.3 前置與提拔/降級雙軌(誠實列明,不假設)

- **單模 8b=預設預算與排程基線**(§3.5 種子即此態):三 tier 全指 qwen3:8b,引擎全功能照跑;此態即舊稿之「降級模式」——現改為**出發點而非退路**(E-6 修訂:舊稿方向倒置)。
- **4b 提拔=受控實驗(#27),雙閘缺一不提**:閘一=`probe_deliberation_models.py` 靜態量測(4b `size_vram==size` 全 GPU+tok/s 回填 verified_note;**兩模同駐時 4b 是否仍全 GPU 屬未知,不得假設**);閘二=**運行期抽查**——提拔試跑期間每 `spot_check_every_n_calls`(config,種子 25)call 打一次 `/api/ps`,size_vram/size 快照落帳本;任一次 <100% 或 tok/s 崩塌 → **自動退回單模**(UPDATE fast.model_tag='qwen3:8b' 一列)並記退場原因(#27 有界+可即時偵測退場)。probe 矩陣增 `--spot-check`(§4.3.3 依此擴)。
- **CPU-only 雙軌(需求 f)**:路由邏輯零分支——Ollama 自行處理無 GPU 情境,引擎只感知 tok/s 變慢;成本模型的 tok/s 取自帳本實測列(GPU/CPU 環境各自累積),不寫死;watchdog stale 閾值隨之重導出(§3.5 規則)。GPU 監控用 `/usr/lib/wsl/lib/nvidia-smi` 絕對路徑(PATH 內無,實查)。

#### 2.3.4 共用 Ollama 治理(與常駐互動服務的資源衝突;E-9)

實測:`augur-chat.service`/`augur-ollama.service` active running;`-np 1` 單 slot=引擎/benchmark 與 chat/advisor **互動請求共用同一序列化佇列**——過夜長跑期間任何互動請求排在長生成後(逐 call 數十秒~數分鐘延遲);雙模實驗態下互動請求(8b)插入 fast(4b)批之間=強制換模顛簸(~9s×2/往返)且打破攢批假設。治理三則:

1. **夜跑窗政策=長跑啟動確認項**(`benchmark_deliberation.py --run-all` 與引擎 nohup 長跑啟動時列印,與 #22 電源確認並列):用戶二擇一並留痕——「夜窗接受互動服務延遲」或「夜窗自行暫停 chat/advisor 服務」(停/啟服務屬外部副作用,#6 由用戶執行,AI 不自行 systemctl stop);未確認不放量(§5.8-7)。
2. **顛簸偵測入帳**:report 統計 `load_duration_ms>swap_thrash_ms` 次數;引擎長跑期間顛簸異常升高=互動流量插入之真兆,報告須列(#15)。
3. **雙模實驗僅限無互動活動時段(夜窗)啟用**;日間一律單模。
...
| 別名/廢名(出處) | 正名 | 裁決依據 |
|---|---|---|
| `deliberation_llm_call`(§4 舊稿) | `deliberation_agent_call`(§3.3.7) | 單一 LLM 呼叫帳本,一表一 role |
...
| **`deliberation_benchmark`(§3.3.12 DDL 字面/§5.5 之基準 run 帳本語境)** | **`deliberation_benchmark_run`** | E-33/E-49 殘項本輪釘死:§3.3.12 字面與 §3.1 MVP 平表**撞名**,`IF NOT EXISTS` 下新表永不建成;migrate 以正名 `deliberation_benchmark_run` 建表,§3.3.12 標題與 CREATE 行、§3.3.13 之 FK 一律讀為 `REFERENCES deliberation_benchmark_run(run_id)`;**§3.1 MVP 平表 `deliberation_benchmark` 同名原樣保留(冒煙帳,不刪不改)——本列僅作用於 run 帳本語境** |
| **suite 枚舉字面 `'plan_gap'`/`'data_qa'`(§3.3.11-12 CHECK)** | **自枚舉刪除**——正典三值=`('seeded_bug','doc_conflict','retrieval_adjudication')` | §5.1 G4:枚舉了不建=假完備;語料設計落地時隨設計回加(migrate 冪等 DROP/ADD CONSTRAINT) |
| **arm 枚舉四值字面(§3.3.12 CHECK)** | **§5.3 七臂表=arm 集唯一 SSOT**(GATE 判定臂集=前六臂) | E-31/E-34/E-40 整合;migrate 冪等 DROP/ADD,`--verify` 斷言 DB 枚舉=§5.3 表 |
...
| `bridge_deliberation_distill.py`(§4.3.6 舊稿)/`export_deliberation_sft.py`(§8.5) | `bridge_deliberation_distill.py(已落地取代原規劃名)`(§6.4) | 與勝出之 §6.2 設計同居所 |
| migrate `--apply/--check`(舊稿) | `--run/--verify`(已落地實名) | 實碼定名 |

**單一定義機械自檢(E-49 收尾;定稿與每次改版必跑)**:①grep 全計畫 `CREATE TABLE`——每表恰一處定義(`deliberation_benchmark_run` 正名後,§3.3.12 與 §3.1 之撞名歸零);②全部表名/script 名/函式名引用 ∈ §3.3・§4.0 正名集 ∪ 本表左欄(一律讀為右欄);③suite/arm 枚舉字面 ∈ 上表正典集。三項全綠才送拍板;此自檢清單即引擎 `doc_consistency` 之 dogfood 題材(P1 MVP 現行即可對本計畫跑)。

**隔離不變式(審議產物絕不入 `knowledge_*`,五道防線)**:

1. **表名前綴**:一切審議/基準產物住 `deliberation_*` 前綴表——此前綴即 `import_isolation.py` 字面掃描之錨(`DELIB_LITERALS` 三擴,同構樣板=`DISTILL_LITERALS` @ src/augur/audit/import_isolation.py:41/:45/:166);**反方向(deliberation import 預測/知識層)由 §4.1.2 `SCAN_DELIB_SRC` 反向掃描機械看守(E-55 補閘)**。
...
> **本表種子分兩波(E-18 時序修正)**:唯 `deliberation_redline_trigger` 一表分「**W1 固化波(P1)**/**D1 後修正波(P3 W3c)**」,其餘表單波即完。P1 dogfood 之唯一已知真陽性=trigger_id=7 檔名漂移,它是**引擎的測題**(§8.1 A1.3 ground truth)——種子不得在引擎啟動前先修掉它;且 glob 化屬判準變更、繫 D1 決策層拍板後才施作(#19/#26 執行層不先斬後奏)。

| 表 | 種子 | 依據 |
|---|---|---|
| `deliberation_model_profile` | **單模 8b 預設(§2.3 修訂)**:`('deep','qwen3:8b',think=false,options={"temperature":0,"seed":42,"num_ctx":4096},timeout_s=300,active=true,verified_at='2026-07-10',verified_note='2.31/5.96GB in VRAM, 6.6-7.7 tok/s, cold load 9.24s')`;`('deep_think','qwen3:8b',think=true,…,verified_note='39.7s/題 vs 5.5s')`;`('fast','qwen3:8b',think=false,options={"temperature":0,"num_predict":512},active=true,verified_at='2026-07-10',verified_note='單模預設;與 deep 同權重零換檔稅')` | 三 tier 同權重、以 think/options 區分——idle VRAM 已被佔 942MiB、「4b 全 GPU」屬邊緣未證(§2.3);**qwen3:4b 不入預設種子**:提拔=#27 雙閘(probe 靜態+運行期 /api/ps 抽查,§2.3.3)通過後 `UPDATE fast SET model_tag='qwen3:4b'` 並附抽查證據入 verified_note(#9) |
| `deliberation_engine_config` | `loop_dry_k=2`、`round_max=6`(保險絲)、`skeptic_n=3`、`majority_arm_n=3`(§5.3 majority_fast 臂票數,與 skeptic_n 對齊)、`vote_kill_rule={"min_votes":3,"refute_ratio":1.0}`(語意 SSOT=§2.1.2 三級制:全票反駁才 discard,殺錯成本>省時故保守)、`schema_retry_max=2`、`refine_topk=5`、`prompt_token_budget=3000`(runner `-c 4096` 實測,留生成餘裕;超過→分塊)、`keep_alive="30m"`(**單模連跑適用**;雙模實驗態 tier 切換顯式 `keep_alive:0`,§2.3.2)、`bench_trials_min=3`(#11)、`pytest_enabled=false`(**D4 安全拍板後才 true**)、`requeue_max=2`、`context_hard_cap=3200`、**`stale_buffer_s=600`**(watchdog 閾值緩衝,規則見本節末段;舊鍵 `heartbeat_stale_min=15` **廢除**)、**`bench_budget_wall_h=80`**(P2 判定批全程牆鐘預算閘,§5.2;操作值 #27 護欄內調)、**`spot_check_every_n_calls=25`**(運行期 /api/ps 抽查,§2.3.3)、**`swap_thrash_ms=1000`**(換檔顛簸判定,§2.3.2/§2.3.4) | 魔數全數住 DB;調參=UPDATE 一列(操作值依 #27 護欄內調整) |
...
| `deliberation_lens` | 初始鏡組(每鏡一失效模式):`skeptic_correctness`/`skeptic_antileakage`(與 redline 4 列 antileakage_column 對接)/`skeptic_truesign`/`skeptic_isolation`/`skeptic_reproducibility`/`skeptic_security`/`critic_completeness`/`judge_risk`/`judge_simplicity`;prompt 全文住 seed script(策展人寫,author='human_curated';**MVP 之 LENS_PROMPTS 三鏡文本遷入,deliberate.py 改讀 DB**);doctrine 檔名引用一律 glob(`docs/原則精華_v*.md`;lens 為新種子非既有 ground truth,無 E-18 時序問題) | 模式3;#29b(現況 hardcode=違例,P3 修);script 為匯入口非 SSOT(同 manual_file adapter 先例) |
| `deliberation_redline_trigger` | **兩波種子(本節導言)**——**W1 固化波(P1)**:現行 9 列**原樣凍結**(冪等固化、不改任何 pattern 值;trigger_id=7 之 `docs/系統架構大憲章_v1.39.0.md` 字面漂移態**刻意保留**=A1.3 ground truth,由引擎自己把漂移列 confirmed);**D1 後修正波(P3 W3c)**:glob 版重建(`docs/原則精華_v*.md`/`docs/系統架構大憲章_v*.md`/`docs/系統核心思想_v*.md`;`pattern` 欄語意本即檔路徑 glob,結構零改)——修正波語句於 seed script 內與固化波分離,**未經 D1 拍板不施作**、屆時由用戶指示執行;§4.3.2「redline 全為 glob 且展開命中」之驗收條款**僅修正波後適用**;4 列 antileakage_column 兩波皆原樣 | **漂移已真實發生**(trigger_id=7 綁 v1.39.0、docs 現行=v1.40.0,本日實證);守 #12(seed script 重建、非手動 UPDATE);時序=**先當測題、後當修復**(E-18) |

**watchdog stale 判定規則(E-7 修正;§7.4 與 `watch_deliberation.py` 一律依本段,舊稿「heartbeat_at<now()-15min 即標」廢除)**:

1. **stale=雙條件聯合**:`heartbeat_at` 逾時 **AND** 該 run/benchmark run 之 advisory lock(§7.3)未被任何連線持有(`pg_locks` 實查)——鎖仍在=進程活著只是單步長,**不標**。消滅假陽性與雙 writer 競態:單步可合法遠超 15min(timeout_s 600×(schema_retry_max+1)=最壞 30min+、CPU-only 8b think 長 prompt、pytest 300s+排隊),舊規則會把活 run 標 failed、原進程隨後照常回寫=兩 writer 改同一列。
2. **heartbeat 打點時機=動作發出前**:每次 LLM call 送出**前**即 UPDATE heartbeat_at(非 step 完成後);oracle 批驗證、pytest subprocess 啟動前同樣先打點——長生成期間心跳恆新鮮。
3. **閾值機械導出、不寫死**:`stale_threshold_s = max(active deliberation_model_profile.timeout_s) × (schema_retry_max + 1) + stale_buffer_s`——watchdog 每輪自 DB 現值導出;CPU-only 環境按帳本實測 tok/s 之單步上界重算(§2.3.3),任何環境不得 hardcode 分鐘數。
4. **標記前 double-check+條件寫**:標 failed 前重讀 heartbeat 與鎖各一次、仍成立才寫;寫入用 conditional UPDATE(`WHERE status='running' AND heartbeat_at = <方才讀值>`),撞不到列=進程剛醒,放行不標(fail-closed 於「不誤殺」一側)。

### 3.6 遷移、冪等與本節驗收

- **`scripts/migrate_deliberation_ddl.py`(已落地,擴充)**:模式實名=`--run`(冪等施作)/`--verify`(斷言);無參數印指令矩陣+現況(#29a,已落地)。`--run` 依序=既有 6 表(已在,IF NOT EXISTS 略過)→ §3.3 新 13 表+§3.4 2 view → §3.2 ALTER → §3.5 種子;全程 `IF NOT EXISTS`/`ON CONFLICT` 冪等,重跑安全。**§3.5 種子之 redline 段,`--run` 恆施「W1 固化波」(原樣凍結)**;修正波(glob 重建)不入 migrate,繫 D1 拍板後由 seed script 施作(§3.5 兩波)。
- **`--verify`(確定性驗收,全綠才算過)**:① **19 表**(6 既有+13 新)+2 view 存在且 CHECK/FK/UNIQUE 與本節逐字一致(`pg_get_constraintdef` 對照);② §3.0 跨界 FK 稽核 query 回 0 列;③ `deliberation_oracle.oracle_name` 集合=verifiers.ORACLES∪{'pytest'} 且 ⊆ claim `assigned_verifier` 枚舉、`lens.task_kinds` ⊆ task `task_kind` 枚舉;④ `model_profile` 無 `active=true AND verified_at IS NULL`(CHECK 已擋,雙驗);⑤ 蒸餾軟連結孤兒稽核:`advisor_distill_question WHERE topic_source='deliberation_engine'` 之 `topic_ref` 均存在於 `deliberation_claim`;⑥ redline `doctrine_file` pattern 之 glob 展開於 docs/ 現存檔非空(防再漂移)——**本項僅於 D1 後修正波施作後生效**(§3.5 兩波):固化波期間印 `SKIPPED(w1-freeze)` 不判 FAIL(trigger_id=7 之字面漂移態=A1.3 ground truth,刻意保留;E-18)。
...
| 2 對抗多數決 | `vote` + `v_deliberation_vote_tally` + claim status `'discarded'`(機械鎖不動;語意=§2.1.2 三級制) |
...
| 15 | `scripts/bridge_deliberation_distill.py(已落地取代原規劃名)` | 新(P5) | 蒸餾映射器(需求 e;§6.4 為規格 SSOT) | 廢名:`bridge_deliberation_distill.py`/`export_deliberation_sft.py` |
...
**錨點覆核留痕(E-53 收尾;2026-07-10 `grep -n` 實查)**:`_assert_local_host` :29、`strip_think` :55、`make_structured_llm_fn` :70(**函式體 :84 呼叫 G3 焊點 `_assert_local_host(url)`——v1.37.0「永不離本機」不變式已內建,舊稿 make_chat_fn 漏焊之慮隨方案廢除而消滅**)、`make_llm_fn` :122(:136 同款焊點)、`advise.py` `def advise` :80/llm_fn 契約 :86、`serve_advisor_openai.py --mock-llm` :42——本節與 §0.2 沿用清單之錨全數成立。**慣例(E-53/E-20)**:承重錨以「函式名+grep pattern」為準、行號僅括號附註(行號隨 diff 漂移,函式名由 file_grep oracle 可重驗);定稿前之「計畫內錨點自驗」即 P1 引擎 file_grep oracle 的 dogfood 題。
...
**驗收**:既有呼叫方(deliberate.py/advisor 全家)行為零回歸(預設 with_meta=False);`with_meta=True` 以最小 schema `{verdict:boolean, reason:string}` 實測回 (dict, meta) 且 meta 四計時欄非空;負測試(G3,E-37):以外部 base 建構須 raise(`_assert_local_host` 焊點,已落地)。
...
**反向掃描 `SCAN_DELIB_SRC`(E-55 補閘;與三擴同批施作)**:現行 `_import_violations`(:63)僅 rglob `PIPELINE` 七 package(:31)掃 `FORBIDDEN` import——**單向**:只擋「預測管線 import 審議」;§4.0「依賴方向單向」之 deliberation 側在此之前**無機械閘**。本擴充補反向:rglob `src/augur/deliberation/` 掃 import 目標,**依賴白名單=`augur.advisor.ollama`+`augur.core`+`augur.audit`**(據實修正 §4.0「僅此二點」為**三點**——verifiers.py 現行即 `from augur.core import db`(:25)+in-process `check_isolation`(:55)之 audit 依賴,實查),命中 `augur.features`/`models`/`universe`/`evaluation`/`ingestion`/`catalog`/`knowledge`/`philosophy` 即 violation;`check_isolation` 接線同構一行。**方向矩陣(誠實列明)**:(a) 預測管線 import deliberation → `FORBIDDEN` 機械擋;(b) 預測管線含 `deliberation_` 字面量 → `DELIB_LITERALS` 機械擋;(c) deliberation import 預測/知識層 → `SCAN_DELIB_SRC` 機械擋(本條新增);(d) advisor→deliberation=**約定+人審**(advisor 不在掃描根,不佯稱機械閘——擴掃描根屬另議,#3 最小邊界)。

**驗收**:`python -m augur.audit.import_isolation` exit 0(現行碼無違例);負向測試三式(測後移除):①任一預測管線檔暫插 `import augur.deliberation` 或 `deliberation_claim` 字面量須 exit 非 0;②deliberation 任一檔暫插 `import augur.features` 須 exit 非 0(反向掃描生效證明);③兩式恢復後重跑 exit 0。此擴充使 `import_isolation` oracle(in-process `check_isolation()`,verifiers.py 實作)自動涵蓋審議層隔離之**雙向**。
...
#### 4.3.6 `scripts/bridge_deliberation_distill.py(已落地取代原規劃名)` — 蒸餾映射器(新;需求 e;**規格 SSOT=§6.2/§6.4,本節僅索引**)
...
**單一權威判準宣告(E-31;§1.5/§4.3.5/§8.2 歸一)**:本節=全計畫預註冊判準**唯一 normative 節**;附屬 SSOT 分工=臂集 §5.3、語料規模 §5.2、計分 §5.4、防作弊 §5.7,各只准一版。他節(§1.5/§4.3.5/§8.2)凡涉門檻、對照臂、臂集、語料規模,一律讀為**引用本節、其自帶數字作廢**——§1.5 之「+15pp/McNemar p<0.05/每型 ≥30」=與本節一致之引用;§8.2 之「A 臂 think:true 對照、seeded-bug ≥20 案」=舊稿冒煙描述,非判準。**判準快照機械斷言**:P5-1 拍板後,判準唯一版本(門檻+臂集+語料規模指紋+scoring_ver+lens 凍結集 sha)快照入每個 GATE run 之 `arm_config`(§3.3.12;run 表名依 §3.0 裁決一律讀 **`deliberation_benchmark_run`**——§3.3.12 字面 `deliberation_benchmark` 與 §3.1 MVP 平表撞名、`IF NOT EXISTS` 下新表永不建成,E-33 殘項本輪釘死,migrate 依此正名建表);`--report-gate` **斷言判準值逐項讀自快照、非 code 常數**,快照缺失或與預註冊版不一致=exit 1——「事後挑最友善門柱」由構造上封死。

| 閘 | 判準 | 依據 |
|---|---|---|
| **G0 suite 效度閘(E-32)** | 每 suite 判定批先跑 `ast_floor`(零 LLM 確定性基線臂,§5.3):其 case-accuracy ≥80% → 該 suite 判「確定性可解=結構性退化」,**不得計入 G1 之 2/3**,語料依 §5.2 構型條款重生成後再入場 | 贏過「一支 AST 腳本即近滿分」的題,證不出 LLM 審議能力;floor 飽和亦必致 discordant≈0 |
| **G1 增量閘** | 三 suite 中 **≥2 個**(且各自過 G0)滿足:engine 臂 vs **全部非引擎臂之最大值**(single_fast/single_think/majority_fast/single_union/ast_floor;經 §5.2 縮編階梯裁撤者除外),**3 個 seed 輪的 median case-accuracy 增量 ≥ +15pp,且對「最佳非引擎臂」每輪成對 McNemar 精確檢定 p<0.05** | E-34:只勝最弱臂=假習得——若輸給 40s think 單發或 lens 聯集單發,依 20/80 立論「習得」即不成立;#11(≥3 次取統計);+15pp 預註冊後不得挪動 |
| **G2 誤報閘** | engine 臂誤報率(負例語料上)≤ 最佳非引擎 LLM 臂 +5pp | 防「多喊多中」假增量(=§1.5 防灌分約束同式) |
| **G3 零外援閘** | 全部臂全程零 Claude token、零外網呼叫(§7.1 機械驗收);**含負測試(E-37)**:以外部 base 建構 `make_structured_llm_fn` 須 raise(`_assert_local_host` :29 焊點,已落地);拔網測試為第二道 | CLAUDE #28;憲章 v1.37.0 G3 隱私鐵律 |
| **G4 裁決權範圍閘(E-42/E-38)** | G1/G2 之效力**逐任務型**發放:code_review←seeded_bug、doc_consistency←doc_conflict、retrieval_adjudication←retrieval_adjudication——該型對應 suite 過閘,才啟用該型裁決權(§1.2 白名單依此讀為逐型附條件)。未基準化型別(**plan_completeness 之結構層以外、data_qa**)一律標 **experimental**:引擎可跑、confirmed 項仍為 oracle 事實,但整體裁決輸出降為 **escalate-only 呈人**、報告不得宣稱已驗證效力。plan_completeness 之「結構層」=零 LLM 機械檢查,並擴充**跨節交叉檢查器**(§4.2.6 `mechanical_coverage` 增項:抽取全文件 CREATE TABLE/表名/script 名/閾值數字,重複定義 byte-diff 比對——「同表多處定義不一致」這類病由確定性檢查抓,不指望 LLM 殘差);num_ctx 4096 分塊審查下之**跨塊語意一致性誠實列為引擎能力外**(歸 experimental) | 防以 seeded_bug 增量替 plan review 背書之擴權(計畫自己警告的病);§3.3.11-12 suite CHECK 之 `plan_gap`/`data_qa` **自枚舉刪除**(migrate 冪等 DROP/ADD;枚舉了不建=假完備,語料設計落地時隨設計回加) |
| **G-fail 條款** | G1 或 G2 不過 → 誠實結論=**繼承前身 §10.2**:只保留確定性子集(P1 產物),§6 蒸餾迴路**不啟動**,結果照樣落表留痕 | #15:失敗也是真兆 |

統計功效誠實條款:McNemar 依賴不一致對(discordant pairs);某 suite 某輪 <10 對=「**樣本不足**」而非通過/失敗,處置=擴語料再跑(機械生成、零 usage),不得以小樣本宣稱習得;**惟 G0 不過(floor 飽和)時擴批無意義,一律重生成語料**(E-32)。

> **拍板點 P5-1**:+15pp / +5pp / p<0.05 / G0 之 80% 屬判準層,用戶拍板後連同臂集、語料規模指紋、lens 凍結集一併快照入 `deliberation_benchmark_run.arm_config`,之後不得中途改判準(改判準=重跑全部輪);`--report-gate` 只認快照(本節首段之機械斷言)。

**分階段規模(E-5 修訂:樞紐拍板不得綁全量——舊稿全量 414 案×4 臂×3 seed 以 §7.5 費率(engine ≈15 分/case)自乘 >400h≈數十夜窗,D2 將無限期延宕)**:GO/NO-GO(§5.1 G 閘)以 **P2 判定批**為判定規模;**P3 全量批**=G1 成立後之穩健性複驗(G-fail 即省下不跑)。

| suite | 測例形 | ground truth 來源 | **P2 判定批(GATE 判定規模)** | P3 全量批(穩健性複驗) |
|---|---|---|---|---|
| `seeded_bug` | unified diff,**多 hunk 構型**(E-32,見下):1 個植入變異 hunk 埋於 ≥2 個良性語意變更 hunk 之間 | 注入器植入清單(機械;每項帶 `defect_type` 粗類) | **40:正 28+負 12** | ≥100:正 70+負 30 |
| `doc_conflict` | 文件段落對(docs/reports 真實段落→植入矛盾);負例=真實編修差異而**無矛盾**之段落對(語意變更、非純排版) | 植入清單(機械) | **30:正 21+負 9**(守 §1.5 每型 ≥30 統計下限) | ≥40:正 28+負 12 |
| `retrieval_adjudication` | 既有蒸餾題:question+context→判 ANSWER/DECLINE/REFUSE | `advisor_distill_question.expected`(S2 確定性模板構造,274 題實measured) | **60(自 274 分層抽樣,label 比例保留、抽樣 seed 固定落 gen_seed)** | 274 全量,零生成成本 |

**seeded_bug 構型與反退化條款(E-32,本條=測例形 SSOT)**:舊稿「純排版負例+被變異行即 diff 變更行」使任務退化為「語意 vs 排版」二分——一支確定性 AST-diff 即近滿分、兩臂同飽和 → discordant≈0、依 §5.1 條款永遠樣本不足,suite 證不出任何審議能力。修訂:
- **正例=多 hunk 真實 diff**:1 個植入變異 hunk+≥2 個**良性語意變更** hunk;良性 hunk 兩個確定性來源(皆零 LLM、seed 可重現):(a) repo git 歷史已合併 commit 之真實 hunk(source-traceable);(b) AST 等價重寫器(變數重命名/常數提取/等價條件改寫,等價性機械可證)。
- **負例=語意變更但正確**之 diff(全部 hunk 皆良性、同上兩源)——**廢除**舊稿「無植入之純排版/註解 diff」定義。
- **ground_truth 每植入項帶 `defect_type`**(粗粒度缺陷類,=變異運算子類之確定性映射;枚舉住 harness 常數並隨輸出 schema 發給**全部臂**,同知識公平)——§5.4 hit 判準之第三條件。
- **held-out 運算子類(E-34)**:運算子分 **disclosed / held-out** 兩組,劃分於 P5-1 凍結、快照入 arm_config;held-out 組=凍結 lens 文本中未出現其名稱/特徵關鍵詞之類(seed script 機械 grep 驗證),判定批占比 ≥25%;報告按組分桶——增量只在 disclosed 組成立=「prompt 策展差」而非編排增量,該 suite G1 不採。
- **lens 凍結(E-34)**:GATE 語料生成**前**,active lens 集之 (lens_name, version, prompt sha256) 快照入 `engine_config.bench_lens_freeze` 並複寫入每 GATE run 之 arm_config;GATE run 啟動時斷言現行 lens 集=凍結集,不一致=exit 1(改 lens=判定輪作廢重跑)。

- **判定批成本冷估(#9 誠實:錨=33-token 玩具探測+§7.5 15 分/case,屬冷估、以 W2c 實測取代)**:engine 臂 full playbook 僅跑 seeded_bug+doc_conflict(70 案×3 seed=210 runs×10-15 分≈35-52h);retrieval 走輕量單輪 playbook(§5.3;180 runs×2-3 分≈6-9h);single_fast/single_think/majority_fast/single_union 四 LLM 對照臂合計 ≈30-45h(union≈single_fast 同級);`ast_floor` 零 LLM、分鐘級≈0;**全程 ≈75-105h≈10-13 個 8h 夜窗**。舊稿 §8.2「2-6 小時級」與 §7.5 費率相差 5-15×——兩處估計一律以本表加總+W2c 校正後投影為準(supersede)。
- **預算閘+縮編階梯(機械規則;E-5)**:預算上限=`engine_config.bench_budget_wall_h`(種子 80h,§3.5)。**W2c 首日校正**:每臂 `--limit 10` 試跑,per-arm median wall 自 run 帳(started_at/finished_at)與 `_result.wall_ms` 實測導出、寫回 run 帳並投影全程;**投影超預算即依序機械縮編並留痕**:① retrieval 抽樣 60→40;② `majority_fast` 臂裁撤(fan-out 消融延後 P3);③ 仍超支=停下問(#26 護欄;**不得**縮 seed 輪——#11 ≥3 不可降;**不得**縮破 §1.5 統計下限;**不得**裁 `single_union`/`ast_floor`——G1 公平性承重,E-34/E-32)。縮編只許在 GATE 判定輪開跑**前**執行(預註冊紀律:開跑後語料/臂/門檻全凍結)。
- **統計功效銜接(§5.1 誠實條款)**:某 suite 判定批 discordant pairs<10=樣本不足 → 機械擴批(該 suite 每次 +20 案,同一生成器同 seed 系列續抽,至功效足或達全量上限);**惟 G0 不過(floor 飽和)=語料退化,一律重生成、不擴批**。擴語料零 usage、不動門柱、非重新設計。
- **futility 節流(預註冊、非挪門柱)**:排程先跑 `ast_floor` 與全部單發臂(便宜、基線先立=§5.8-2)→ engine 臂 seed 輪 1;若輪 1 對當時最佳非引擎臂之增量 <+5pp(門柱 +15pp 之 1/3)→ 印 NO-GO 預警並停下問是否投入輪 2/3(判準不變,僅節流止損)。

**seeded_bug 變異運算子**(確定性、seed 落表可重現 #10;disclosed/held-out 分組依上文條款凍結):比較運算子翻轉(`<`↔`<=`、`==`↔`!=`)、off-by-one、None 檢查刪除、`and`/`or` 對調、早退 return 移位、**字串 SQL 欄名改名**(前身 H3「字串 SQL 掃描為 #8 命門第二面」)、**時點欄誤用**(as-of 條件改未來欄——#8 專項,對 `column_catalog.anti_leakage_flag` 標記欄取樣)。
...
> **臂集修訂(E-32/E-34/E-40 整合)**:節名「四臂」保留為歷史錨,本表現為**七臂**=四 LLM 對照臂+公平聯集臂 `single_union`+零 LLM 確定性臂 `ast_floor`+P3 消融臂 `engine_no_vote`;**GATE 判定臂集=前六臂**,全文他處「四臂」字樣(含 §5.8-3)一律讀為本表 GATE 臂集。**DDL 落點**:§3.3.12 `arm` CHECK 枚舉依本表讀為七值,由 migrate 冪等 DROP/ADD CONSTRAINT 施作,`--verify` 斷言 DB 枚舉=本表 arm 集(臂集唯一 SSOT 仍為本表,E-31)。

| arm | 內容 | 目的 |
|---|---|---|
| `single_fast` | qwen3:8b 單發,`think:false`,`temperature:0` 顯式覆蓋(預設 0.6 實測,不可依賴) | 成本基線 |
| `single_think` | 同上 `think:true`(實測 ~7× 耗時) | 「深思單發」是否已夠 |
| `majority_fast` | **N=3** 次 `think:false`、`temperature:0.6` 多數決(修訂:與 engine `skeptic_n=3`、§8.2 B 臂對齊;5 票在物理序列化下成本過高且無對照必要) | 分離「fan-out 本身」的貢獻;§5.2 縮編階梯②觸發時本臂裁撤、消融延後 P3 |
| `single_union`(新,E-34) | qwen3:8b 單發,`think:false`、`temperature:0`;prompt=**凍結 lens 集全部指令之聯集**(§5.2 凍結條款;同知識、一次呼叫)。誠實物理界:聯集 prompt 超 `prompt_token_budget` 時分塊多發並記 `chunked=true` 入 arm_config(仍計單發臂成本軸) | 公平單發臂:排除「engine 贏在 lens prompt 策展差、單發只拿泛用 prompt」之替代解釋——engine 增量須在同知識下仍成立 |
| `ast_floor`(新,E-32) | **零 LLM 確定性腳本**,逐 suite 一支:seeded_bug=AST-diff 簽名器(逐 hunk 語意變更偵測+變異簽名規則);doc_conflict=數值/條號/日期 diff 掃描器;retrieval=多數標籤常返器。輸出同形 JSON;`model_tag='deterministic:v1'`(#1 溯源)、`llm_calls=0` | G0 效度閘料源+G1 比較集下界:引擎必須同時勝過「不用 LLM 的工具」,否則該 suite 測的不是 LLM 能力 |
| `engine` | 完整審議引擎(多鏡+工具實證+loop-until-dry);**playbook 依 suite 定,見下表** | 受測者 |
| `engine_no_vote`(新,E-40;P3) | engine 去投票消融(vote 機制關閉,其餘同 engine)。**GATE 判定批不跑**;G1 成立後之 P3 穩健性輪跑,量測 vote 之淨貢獻與「殺真 claim」率(共模假陰性)。並釘死 §2.1.2 讀法:其 (b) 之「無 oracle 可裁」繫於 **claim 本身**——帶合法錨+已派確定性 verifier 之可驗 claim **一律先過 oracle(毫秒級,經濟上恆先行)、永不因票 discard**;票僅對純意見項作 discard/escalate 排序(花 3 次 LLM call 省一次 grep=負收益,禁) | E-40:同底模懷疑者相關錯殺屬未量測風險;以消融數據決定 vote 機制去留 |

**engine 臂 per-suite playbook(E-5 成本錯配修正;playbook 名落 `deliberation_run.playbook`、快照入 `_run.arm_config`——GATE 對比恆在「同 suite 同 playbook」下成立,預註冊後凍結)**:

| suite | engine 臂 playbook | 理由 |
|---|---|---|
| `seeded_bug` | `draft_attack_refine`(多鏡多輪,round_max=6) | 缺陷定位=引擎全功能主戰場 |
| `doc_conflict` | `skeptic_majority`(多鏡,round_max=3) | 段落對矛盾偵測不需 refine 迴圈 |
| `retrieval_adjudication` | `single_shot_adjudicate`(單鏡單輪+oracle 錨定,round_max=1) | 單一三值分類題配多鏡多輪=成本錯配(15 分/case vs 2-3 分/case);此 suite 檢驗的增量假說=「oracle 錨定」而非「多輪」 |

**模型基線(§2.3 修訂之落點)**:**LLM 臂全數以單模 qwen3:8b 跑**——單發臂與 engine 臂同權重,增量歸因純落在編排層(20/80 立論的乾淨對照,且消滅換檔稅混淆變因);`ast_floor` 零模型(model_tag='deterministic:v1' 天然分帳)。4b 若經 #27 雙閘提拔,屬另一 `model_tag` 之獨立 run 系列(`_run` UNIQUE 含 model_tag,天然分帳),不與 8b 結果混併宣稱。

LLM 臂統一走 `format=json schema` 結構化輸出(make_structured_llm_fn 已落地),schema:`{findings:[{file,line,defect_type,description}], no_defect:bool}`(suite 3 為 `{label}`;`defect_type` 枚舉=§5.2 粗類,隨 schema 發給全臂=同知識,E-32/E-34);`ast_floor` 由確定性腳本輸出同形 JSON。同臂同 suite 共用固定 prompt 前綴(prompt cache 實證生效)。

- **seeded_bug / doc_conflict**:hit=finding 之 `file` 相同 **且** `|line−line_gt|≤3` **且** `defect_type` 與 ground_truth 該植入項之粗類**枚舉等值**(E-32:三條件缺一不 hit——舊稿僅位置雙條件,指著任何語意變更行即得分;defect_type 為機械字串比對、仍零 LLM,`description` 語意仍不比對——語意比對即重新引入判讀);false_alarm=指向無植入位置、或位置對而 defect_type 錯;**case-level correct**=全部植入項皆 hit 且 false_alarms≤2;負例(語意正確之重構 diff,§5.2)correct=`no_defect=true` 且 findings 空。
- **retrieval_adjudication**:correct=label 三值全等。
- **`ast_floor` 臂同規則計分,零特權**(其 G0 效度閘/G1 下界角色=§5.1/§5.3);`single_union` 分塊發送(chunked)時 findings 取聯集後同規則計分。
- **McNemar 精確檢定**:對每(engine, **最佳非引擎臂**——§5.1 G1 比較集)同 seed 輪成對,b=engine 對/對照錯、c=engine 錯/對照對,p=min(1.0, 2·BinomCDF(min(b,c); b+c, 0.5))——純標準庫 `math.comb`,零新依賴。
...
engine 臂的價值主張=「工具實證勝過意見」,故它**可以**用 grep/psql/pytest 驗證 diff 宣稱——但不得直讀答案、**也不得繞道被取樣原檔「diff 出」答案**。**防線次序修訂(E-36/E-51):裁決性防線=DB 權限層之機械否定,模板/denylist 降為第二道**——「機械否定制、非 prompt 自律」(§2.2.5 creed)之落實;**本節=benchmark 防作弊權限 SSOT,§2.2.2 依此讀**(舊稿「db_query 為模板制、ground_truth 不在模板可及範圍」之宣稱作廢:§2.2.2 之自由單條 SELECT 形制不變,擋人靠權限)。機械落實:

- **第一道:權限層否定(E-36)**——benchmark 模式下 `db_query`/`information_schema` oracle 一律改連第二角色 **`deliberation_bench_ro`**:**顯式白名單逐表 GRANT(絕不 `GRANT ALL`)**,白名單由 migrate 自 `information_schema.tables` 導出並機械排除禁區;禁區=`deliberation_benchmark_case`/`deliberation_benchmark_run`(§3.0 讀名)/`deliberation_benchmark_result`/`advisor_distill_question`/`advisor_distill_context`,並防禦性 `REVOKE ALL`。§2.2.2 之 `deliberation_ro`(`GRANT SELECT ON ALL TABLES`,含 ground_truth/expected 可及)**僅供非 benchmark 運行**;benchmark harness 啟動時**斷言連線角色=bench_ro**,否則 exit 1。角色 DDL:

```sql
CREATE ROLE deliberation_bench_ro LOGIN PASSWORD '<住 .env,不入 git>';
GRANT CONNECT ON DATABASE augur TO deliberation_bench_ro;
GRANT USAGE ON SCHEMA public TO deliberation_bench_ro;
-- 白名單逐表 GRANT 由 migrate 生成(information_schema 全表 − 禁區表);絕不 GRANT ALL
REVOKE ALL ON deliberation_benchmark_case, deliberation_benchmark_result,
           advisor_distill_question, advisor_distill_context FROM deliberation_bench_ro;
ALTER ROLE deliberation_bench_ro SET default_transaction_read_only = on;
ALTER ROLE deliberation_bench_ro SET statement_timeout = '30s';
```

- **第二道:oracle 層 denylist**——`db_query` 於 benchmark 模式加禁區述詞(表名 denylist,verifiers.py 擴充;regex 有繞過面,故僅輔助、非裁決防線)。
- `file_grep` oracle 於 benchmark 模式禁讀 `benchmark_deliberation.py` 與其變異運算子模組(自身含植入邏輯)。
- **取樣原檔禁區(E-25 混淆變因設防)**:benchmark 模式 file_grep **逐 case 禁讀該 case 之 `source_ref` 路徑**(自 `deliberation_benchmark_case.source_ref` 機械注入 denylist,零人工)——不擋則 engine 臂只需 grep 被取樣原檔,即可對 payload 逐字比對「diff 出」全部植入(seeded_bug 之 src 原檔、doc_conflict 之 docs 原段落皆然),一支確定性 diff 腳本可零 LLM 100% 命中、單發臂無工具必輸;屆時 +15pp 量到的是「工具可及性」而非「多視角對抗+迭代」,「習得」宣稱被結構性灌水(#15 樞紐風險)。禁區生效後兩臂對 payload 資訊對等(engine 仍可 grep 其餘 repo/查 DB 驗宣稱),增量歸因才乾淨落在編排層。
- 驗收含**作弊探測例**三型:(i) ground truth 只存在於禁區(payload 內無任何線索)的 case——engine 若答對即證洩題;(ii) **原文洩題探測例**——植入僅能靠 diff `source_ref` 原檔逐字比對發現之 case,engine 若命中即證 source_ref 禁區失效;(iii) **權限探測例(E-36)**——benchmark 模式下對禁區表發 SELECT 之 claim,oracle 須回權限錯誤→undecidable,engine 若取得任何禁區資料即證角色切換失效。任一型命中,harness 判 FAIL。

### 5.8 驗收(W 階段對應見階段規劃節)

1. 三 suite 語料落表達 §5.2 **判定批**規模(全量批於 P3 前落齊),`gen_seed` 全填,同 seed 重跑 byte-identical(#10)。
2. `single_fast` 臂三 suite 判定批完跑 ≥1 seed 輪(基線先立,engine 未成也有真兆基線)。
3. 四臂×3 seed 輪跑完判定批,`--report-gate` 自動輸出 G1/G2 判定,無任何人工計算。
4. 作弊探測例判 PASS(engine 拿不到禁區)。
5. G-fail 時:報告照常產出留痕,計畫依 §5.1 G-fail 條款收斂——**這也是合格的驗收結果**。
6. MVP 平表冒煙迴歸(`--run`)於每次引擎重大改版後重跑,防退化。
7. **長跑啟動確認(E-9)**:`--run-all` 啟動時列印夜窗政策二擇一(§2.3.4)+電源確認(#22)+預算閘現值與最新投影,未經用戶確認不放量;確認選擇留痕於 run 帳。
8. **首日校正留痕(E-5)**:每臂 `--limit 10` 試跑之 per-arm median wall 寫回 run 帳並入 report;投影超 `bench_budget_wall_h` 時,縮編階梯(§5.2)之觸發、選項與時點有帳可查,且發生於 GATE 判定輪開跑前。
9. **換檔/顛簸稽核(E-6/E-9)**:report 含 `load_duration_ms>swap_thrash_ms` 顛簸計數——單模預設下應 ≈0,異常升高=互動流量插入或雙模殘留之真兆,報告須列;雙模實驗 run 另附運行期 `/api/ps` 抽查快照(§2.3.3)。
...
**train-on-test 禁區(E-35;永久 held-out,自毀證偽儀器之防)**:`deliberation_benchmark_case` **全數排除於蒸餾出口**——舊稿映射表第 4 列(benchmark_case seeded_bug 正例→ANSWER gold)**刪除**;且映射器述詞**機械排除 benchmark 情境產生之 claims**:`run_kind='benchmark'` 之 run 所屬 claims、及任何 `deliberation_benchmark_result.engine_run_id` 溯及之 run 之 claims,一律不出口(測集內容經 engine 臂審議滲入 SFT=同型洩漏)。**稽核(機械)**:`bridge_deliberation_distill.py(已落地取代原規劃名) --run` 前置斷言+`--stats` 報告——已出口 question(topic_source='deliberation_engine')之 topic_ref 所指 claim 不得屬 benchmark run、亦永不指向 benchmark_case(=0 列,>0 即 loud fail;併入 `migrate --verify` 之蒸餾軟連結稽核項)。理由:一旦(另案 P6-2)微調發生,基準若曾入 SFT,P3「重跑 P2 基準」與一切後續評測都在評看過測集的模型——本計畫唯一的機械裁決法庭從此失效;本禁區即該儀器之永久保全,**微調後評測一律另立 held-out(P6-2 另案範圍)**。**§6.6 驗收 2 隨本條改讀**:「`deliberation` 樣本 ≥1 批落表+上述稽核=0 列」(benchmark_case 蒸餾批作廢;advisor-QA 原 274 題零汙染驗證不變)。
...
**`scripts/bridge_deliberation_distill.py(已落地取代原規劃名)`** — 三元組→QA 形映射器(零 LLM;**正名,取代舊稿 bridge_deliberation_distill.py/export_deliberation_sft.py 兩廢名**;§8.5 W5a 所指之映射層即本支)。
...
```
python scripts/bridge_deliberation_distill.py(已落地取代原規劃名)                          # 印矩陣+可映射存量統計
python scripts/bridge_deliberation_distill.py(已落地取代原規劃名) --dry-run --limit 10     # 列映射樣本(question/context/gold)不落庫
python scripts/bridge_deliberation_distill.py(已落地取代原規劃名) --run --batch-tag delib_20260801
python scripts/bridge_deliberation_distill.py(已落地取代原規劃名) --all-confirmed --batch-tag delib_v1   # 全帳本掃描(冪等)
python scripts/bridge_deliberation_distill.py(已落地取代原規劃名) --stats                  # 各 batch_tag 之 S5 通過率
```
...
| E-4 | blocker | 程式清單三套未 reconcile,依 v1.39.0「每支程式用途+CLI 矩陣」標準不可拍板:蒸餾橋三個名字(§4.3.6 bridge_deliberation_distill.py/§6.4 bridge_deliberation_distill.py(已落地取代原規劃名)/§8.5 export_deliberation_sft. | 以 §4 為程式規畫唯一權威節,§5.6/§6.4/§8 全部改引用 §4 檔名;模組佈局二選一(建議 §4.0 六模組,§2.0 表刪除);distill staging vs export 依 §6.2 裁決(方案 A、零新表)回改 §8.5 W5a。 |
