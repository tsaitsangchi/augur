# augur 通用 ingester — Live API 驗證 schema catalog v2 (2026-06-11, v1.4.0)
...
## 總計（v1.4.0：唯一資料本質排除＝#4 日為最小單位）

- FinMind dataset 全集(`list_datasets` enum)：**93**

- 真排除(① intraday <日,#4 唯一資料本質準則)：**8**

- operational 暫緩(② `OUT_OF_UNIT` 規模物理不可行,#3,非治權排除)：**3**(券商分點/權證/鉅額)

- 候選可抓 = 82；**live 取樣成功建 schema = 81 表**；未取樣 = 1

- 其中經 **#18 by-維度-id(/datalist)** 補抓 = 2 表(總經/全球類:幣別/期別/央行/油品)

- **可建表合計：81 FinMind 表 / 662 欄 + FRED `fred_series` 1 表 / 3 欄 = 82 表 / 665 欄**
...
- 取樣 mode：2nd:{'data_id': 'TAIEX', 'start_date': '2015-01-01'}；PK：`['stock_id', 'date']`
...
- 取樣 mode：special-id(TX)；PK：`['futures_id', 'date']`
...
### `CrudeOilPrices`  (3 欄)
- 取樣 mode：by-維度-id(/datalist,2個id)；PK：`['date']`
- 維度 id（`/datalist`，2 個）：Brent, WTI
...
### `GovernmentBondsYield`  (3 欄)
- 取樣 mode：by-維度-id(/datalist,13個id)；PK：`['date']`
- 維度 id（`/datalist`，13 個）：United States 1-Month, United States 1-Year, United States 10-Year, United States 2-Month, United States 2-Year, United States 20-Year, United States 3-Month, United States 3-Year, United States 30-Year, United States 4-Month, United States 5-Year, United States 6-Month, United States 7-Year

| # | 欄位(API 原樣大小寫) | 型別/大小 | 中文 | PK |
|---|---|---|---|---|
| 1 | `date` | DATE | 日期 | ✔ |
| 2 | `name` | VARCHAR(255) | 債券名稱 |  |
| 3 | `value` | NUMERIC(20,6) | 殖利率 |  |
