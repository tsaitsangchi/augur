...
## 二之二、適用範圍界線：非治理輔助專用（硬邊界）

**本工具僅供「非治理輔助」用途，不得作用於治理權威語料。** 此為回應本專案「不採信自陳、對抗審查方為關卡」之鐵律（`LAYER-SEALING-SCHEDULE.md`：定律已 12/12 應驗、建造者自查零攔截）與 P2.Y 幻覺風險而立的**設計上界**。

| 語料分區 | 範圍 | 由誰處理 |
|---|---|---|
| **治理權威語料（排除）** | `constitution/`（MC、`RULING-*`、`AMENDMENT-LOG.md`、`adoption-drafts/`）、**生效** `specs/*-SPECIFICATION.md`（無 `-draft`） | **僅** `constitution-mcp`（精確原文、零 LLM、零幻覺） |
| **非治理輔助語料（適用）** | `reports/`、`audits/`、`ops/`、`infrastructure/`、程式碼、`*-draft` 草案、README 等 | 本 `local-llm-mcp`（[I] 濃縮） |

**強制手段（硬編、selftest 斷言）**：`local_summarize`／`local_extract` 收到落在**治理權威語料**內之路徑時，**拒絕執行並回顯式 `isError`**，訊息導向 `constitution-mcp`（「治理條款之精確原文請經 constitution-mcp / get_clause，本工具不對生效治理語料做 LLM 濃縮」）。理由：該語料 `constitution-mcp` 已以確定性檢索完整覆蓋，疊 LLM 摘要**只增幻覺風險、零邊際價值**，且正中 WM.44-LABEL「以轉述冒充原文」之病灶。
...
5. **治理語料排除**（見 §二之二）：對落在治理權威語料（`constitution/`、生效 `specs/*-SPECIFICATION.md`）內之路徑，`local_summarize`/`local_extract` **拒絕執行並回 `isError`**，導向 `constitution-mcp`。以路徑前綴判定，selftest 以正/反例逐項斷言。
...
| 2 | 治理護欄硬編（來源標記、失敗發聲、路徑封閉、不快取、**治理語料排除**） | 同上 | selftest 逐項斷言（含治理路徑正/反例） |
...
## 九、元憲章對齊（依 constitution-mcp 實查 2026-07-21；AUGUR-MC v1.4，附 `file:line`）

本工具為 [I] 輔助、不創設 [N] 義務；其設計逐項對映元憲章：

| 本工具設計決策 | 元憲章依據（出處） |
|---|---|
| 本地 LLM 輸出一律 [I]、**禁入 [N] 治理文書** | **P2.E2**：Model output 不得未經 Evidence 通道（§2.11）直接成為權威 Representation 或 Knowledge（`META-CONSTITUTION.md:221`）；**P2.E1**：禁 AI 直接從 raw data 建立永久 Knowledge（`:215`） |
| **治理權威語料排除**（§二之二：不對生效治理語料做 LLM 濃縮，導向 `constitution-mcp`） | **P2.E2**（`:221`）＋**P2.E4**：禁止 Representation 被視為 Reality 本身、須保留 Observation 來源與不確定性（`:223`）——LLM 摘要即 Representation，不得覆蓋治理原文 |
| **為何必須有此護欄**（幻覺風險） | **P2.Y**：「AI 最大風險不是能力不足，而是對錯誤世界產生高度合理的智慧」（`:205`）——本工具 `local_ask` 誤答「以太坊預測市場」即實例 |
| 來源標記強制（provenance） | **P4.E1** Knowledge 五元組含 **Source**（`:293`）；**P3.E1** 引用與解析義務（`:265`） |
| 唯讀、不給 agent 經 MCP 之寫入旁路 | **§8.1**：Steward 持修憲裁決權、**Agent 不得參與**（`:477`） |
| 失敗發聲、不靜默降級 | **P2.E5（Fail-safe）**：錯誤/撤回時衍生 Knowledge 必須連動失效（`:224`） |
| 深度推理/最終判斷回 Cursor（人） | **P2.W1/W2**：權威順序（非時間順序）；AI 得參與建構但非權威（`:197`/`:201`） |
| 全案定位（五大原則） | **§9 終極宣言**：Reality First／Representation Before Intelligence／Identity Before Knowledge／Evidence Before Conclusion／Accountability Before Action（`:535`） |
