...
| E2 M2 EVT | ξ＋95% CI 落庫；n_tail 揭露；selftest **ξ 之 95% CI 覆蓋真值**（統計正確判準）＋點估誤差 <20% @n_tail=1500 | n_tail<20 拒答 |
| E3 M3 copula | 邊際存活數/dof/尾部相依落庫；selftest 還原合成相關誤差<0.05 | 邊際收斂失敗>25% → 停手報告不硬出 |

**回滾**：全部為新增 method 之新列（append-only）；不動既有 run、不動主結論層。


## 九、E2 驗收判準更正（2026-07-27 實證後）

原文「selftest 還原合成 ξ 誤差<10%」**規格不切實際**——GPD MLE 之點估誤差隨尾部樣本數變：實證 n_tail=300／1500／6000 時誤差 24.8%／11.6%／6.8%（單調收斂、CI 三次全覆蓋真值 0.25）。故判準改為**CI 覆蓋真值**（統計上正確、與 n 無關）＋輔以 n_tail=1500 之 <20% 收斂檢查。此為判準**修正**非放寬：原判準在任何合理 n 下都不可能通過，形同無效條款。

並記一 bug：EVT 路徑初版寫 `exp(cumsum)`（淨值）而非既有引擎約定之 `exp(cumsum)-1`（累積簡單報酬），使 MaxDD 被系統性低估、EVT 反比 iid 溫和。修正後 P(MaxDD<-20%) 由 0.69% 升至 **2.57%**（iid 0.33% 之 7.8 倍），厚尾簽名正確。
