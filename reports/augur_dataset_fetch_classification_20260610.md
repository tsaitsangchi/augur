...
| **BlockTradingDailyReport** | 單日 97 列 | **by-date 抓** | 全市場/日（鉅額交易事件）|
...
> 核心原則（呼應靈魂「系統單位 = 個股 × 日」+ #4 時間最小單位之實體延伸）：**raw 層收「個股×日」或粗類別標準信號（如 3 法人）；sub-stock 券商分點明細 + 非股標的（權證）為範圍外排除**。

## 全量 + 增量一致（3 層守 OUT_OF_UNIT）

| | 排除 OUT_OF_UNIT | GovBank/BlockTrading by-date |
|---|---|---|
...
→ 任何入口（列舉 / 全量 / 增量 / 直接點名）皆一致排除券商分點與權證；GovBank/BlockTrading 全量與增量都抓得到。
...
- `src/augur/ingestion/ingest.py`：`OUT_OF_UNIT` 排除清單（券商分點 + 權證）。
- `src/augur/ingestion/sync.py`：`daily_datasets` 過濾 + `sync_finmind_dataset`（早退 + by-date fallback resume-aware）+ `sync_by_date`（早退排除）。
- `docs/原則精華_v1.2.0.md`：#3 ENFORCE 範圍外明文入憲。
