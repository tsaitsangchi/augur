...
* **性質**：[I] plan-first 計畫書（CLAUDE #16／#20；領域大憲章第六部計畫完整性 v1.39.0）— **不創設 [N] 義務**；**本輪僅計畫；執行未開**
* **授權觸發**：Steward「**開 R7 計畫**」＝只寫本檔（本輪）；「**開 R7**」／分階碼＝實作另授權
...
|---|---|
| **日期** | （待 Steward） |
| **建議碼** | `R7-P-yes` ＋ `R7-G12` ＋ `FZ-keep` ＋ **上線政策引用 `PME-AUTO-B`**（見 §10） |
| **效力（拍板後）** | **計畫採納**為 R7 閘框架藍圖；**實作另待「開 R7」／分階碼** |
...
| **R7-G0** | 僅 S0 盤點留痕（產品索引＋缺口表） |
| **R7-G1** | 授權 S1：閘檢查清單機械化（零產品實作） |
| **R7-G12** | 授權 S1＋S2：閘機械化＋掛接**一個**首選產品計畫（仍禁該產品全量綠地重寫）— **建議近程** |
| **R7-Gfull** | S0–S2＋U7 一次授權（仍逐產品另開執行；不併吞全產品） |
| **FZ-keep** | 不解凍 FinMind／FRED；Dividend 維持 PAUSED |
| **PME-AUTO-B** | **引用** PME 上線政策（有界自動＋kill-switch）；非本檔重立法；正式採納仍待 PME 自身拍板碼 |
...
| 上線政策對齊 | PME §2／§4.1 | R7 敘事／驗收**引用 PME-AUTO-B**；不另立法「人准上線」 | PME 意向已出；正式碼待回 |
...
* PME-AUTO-B 已定意向：若 R7 閘文寫死「特徵上線一律人准」，會與 PME 衝突並逼搶改靈魂——**R7 只引用、不重寫、不搶 [N]**。
...
| **本輪實作 S1／S2／U7** | 「開 R7 計畫」＝只 plan-first |
...
| **近程狀態** | DONE（≠可交易） | S1＋S2＋U6 DONE（≠可答完備） | 計畫已出；待 `PME-P-yes`＋`PME-AUTO-B` | **計畫本輪；執行未開** |
| **上線／準入** | direction／arena 門柱；人 approve gate | source activate／HAR-ext 人裁 | **PME-AUTO-B**：機械閘全綠→引擎上線；人＝監控＋kill＋治權 | **引用 B**；不另定「人准特徵上線」 |
| **API** | FZ-keep | FZ-keep；HAR-ext≠FinMind | FZ-keep | **FZ-keep** |
| **ultracode** | U5 ✅ | U6 ✅ | 產品內階段對抗（PME 自訂） | **U7**＝閘框架／首掛產品拍板前 |
...
| **P-PME** | `reports/augur_philosophy_market_evolution_loop_plan_20260724.md` | 哲學↔市場進化閉環 | **上線＝PME-AUTO-B**；待 `PME-P-yes`；**建議首掛**（政策 SSOT 最近、與 R7 交界最清） | **建議 S2 首掛** |
...
| PME 正式拍板 | 待碼 | 引用意向 B；正式碼與 R7 可同批或分批 |
...
| **輸出** | 可重複跑之閘檢查入口（建議 `scripts/verify_roadmap_r7_gate.py`：`--check` 對**指定計畫 path** 做結構／關鍵節存在性／禁語抽樣；`--selftest` 零 DB）；檢查清單 markdown 模板；路線圖／本檔證據路徑 |
...
### S2 — 掛接首個產品（示範過閘，非全量實作）

| | |
|---|---|
| **輸入** | S1 哨兵綠（或誠實欠項＋Steward 豁免）；首選產品（預設 **P-PME**） |
| **輸出** | 該產品「閘通過紀錄」`audits/ROADMAP-R7-GATE-<PRODUCT>-*.md`（G-P* 逐項證據）；產品計畫補齊 v1.39 缺口之**修訂**（仍 [I]）；**不**自動等於「開 PME／開顧問實作」 |
| **風險** | 掛接＝偷跑產品實作；或掛接後宣稱 R7＝該產品 DONE |
| **停手** | 無 `PME-P-yes`（若首掛 PME）卻開始閉環 code；打 FinMind |

**掛接定義（鎖定）**：補齊閘文件＋哨兵對該 path PASS（或登錄豁免）＋四判準書面 → 產品進入「**可申請執行授權**」狀態；執行仍須用戶「開 〈產品〉」或產品專屬碼。
...
> 回覆字串即可登錄為本計畫授權（**不**自動等於「開 R7 實作」除非含執行項）。
>
> **本輪狀態**：計畫已出；**待回碼**；執行未開。
...
- **〔R7-G0〕** 只做 S0 盤點留痕。  
- **〔R7-G1〕** 授權 S1（閘機械化；零產品功能實作）。  
- **〔R7-G12〕** 授權 S1＋S2（機械化＋掛接首個產品；預設 P-PME，可註改）— **建議近程**。  
...
- **〔PME-AUTO-B〕** **引用**：特徵／原則狀態上線政策以 PME 之有界自動＋緊急停為準；R7 閘 G-P6 對齊；**不**在 R7 寫死一律人准；靈魂措辭另案（不本輪改 [N]）。  
- （可選）**〔PME-KILL〕** 強調 kill-switch 為 B 硬要件（與 PME 計畫同義）。
...
**建議下一句（用戶）**：回 `R7-P-yes + R7-G12 + FZ-keep + PME-AUTO-B`；之後「**開 R7**」才進 S1／S2——**仍禁** API 解凍、**仍禁**假完備宣稱。
