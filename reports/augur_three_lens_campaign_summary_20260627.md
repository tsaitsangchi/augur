...
## 十一、phase-2 建置結果(2026-06-28)→ gate 留、特徵全淘汰、**深度飽和定論**

- **發布日 gate(keeper)**:`features/release_lag.py`(月營收次月15/財報+45/90日)+ 修 monthly_revenue_yoy 洩漏。驗證:gated IC +0.0476/HAC-t 3.14 ≈ 洩漏 +0.0492(特徵本就誠實)、headline 守住 +0.1411。**#8 淨賺、可重用**。
- **P5+C3 候選(5 個)過漏斗 → 全淘汰**(`verify_fundamental_candidates`):
  | 候選 | H60 HAC-t | 共線 | 多因子增量 |
  |---|---|---|---|
  | x_gross_margin_pctile | 2.08(過) | **低(正交)** | 單獨 H60 Ridge +0.002、3/4 ≤0 → ❌ |
  | x_revenue_share_mom | −3.02(負號) | 低 | 組內負 → ❌ |
  | 營收share/庫存ratio/Δ | <2 | 低 | ❌ |
- **深度飽和定論(#15)**:phase-2 是**真正交新資訊(全低共線 |r|<0.5)卻仍零增量**——不只「重組類」失敗,**連全新基本面維度(營收支配/財報循環)亦不增量**。原因:季/月頻稀疏、訊號弱(|IC|≤0.055)、GBDT 已從既有特徵榨出可預測內容。**34 特徵模型已達此宇宙/H20-60 之資訊前沿**。
- **五輪探索全淘汰**(撒網/A交互/B馬太/當沖/phase-2)→ 特徵層飽和**深度確認**;前瞻價值已不在更多特徵,而在**經濟/執行層**(已證 alpha 扛成本)、不同宇宙/horizon、或 regime-conditional(C1 已驗、見 §十)。
...
## 十二、horizon 掃描(2026-06-28)→ **逃脫飽和:alpha 在長 horizon 更強**

特徵層五輪飽和後,換 horizon(非加特徵)掃描模型 IC(as-of core、3 seed):

| H | Ridge IC | GBDT IC | 動能 |
|---|---|---|---|
| 5 | +0.129 | +0.082 | −0.002 |
| 20 | +0.125 | +0.108 | −0.004 |
| 60 | +0.141 | +0.112 | +0.017 |
| **120** | **+0.152** | +0.134 | +0.032 |
| **252** | +0.151 | **+0.149** | +0.038 |

**發現**:**IC 隨 horizon 單調上升**(Ridge H60 +0.141→H120 +0.152;GBDT +0.112→+0.149、增 33%)——這批特徵(估值/循環/相位)是**長 horizon 訊號**,現用 H20/H60 低估 alpha。

**H120 經濟回測(非重疊半年、2014+、17 期、扣 0.585%)**:Ridge top10%/equal 淨 **Sharpe 1.27 / Calmar 3.55 / MaxDD −5.3% / CAGR +18.8%**(vs H60 季 Calmar 1.32/MaxDD −14.8%;vs 基準 Sharpe 0.95/Calmar 1.03)。**低換手(半年~1.65×/年)→ 成本幾乎不咬(net≈gross)**、回撤大降。caveat:窗異(H120 用 2014+ vs H60 用 2021+)、17 期小樣本 → 確切 MaxDD 偏樂觀,但低換手/高 IC 之結構優勢穩健。

**宇宙掃描(訓練 core → 測 core 461 vs 擴大 1117、H60)**:模型 IC core +0.141(Eff-t 6.28)→ **擴大 +0.120(Eff-t 5.66、2.4× 股數)**——**alpha 推廣到更廣宇宙、IC 僅降 ~15%**(pb_ratio 在擴大更強=小型股價值)。**可交易宇宙可擴、容量更大。**

**戰略結論**:34 特徵**特徵層飽和**(五輪),但**換 horizon(半年)+ 擴宇宙(2.4×)雙雙解鎖價值**——更強 IC、更低成本/回撤、更大容量。**前沿不在加特徵,而在 horizon × 宇宙 × 執行配置。** 工具 `run_horizon_universe_scan.py`、`run_economic_eval.py`(非重疊)。
