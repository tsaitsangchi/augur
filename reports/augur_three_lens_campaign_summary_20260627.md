...
- **長線**:跨 phase 2 軸需 infra——八二 P5 營收支配(IndustryChain+P-lag)、康波 C3 基本面循環(財報+P-lag)、C1 景氣(**表缺、須先匯入國發會指標**)。
- **真交互(非線性)**:若再探跨鏡,須乘積/比值/條件型且證勝 GBDT 自學(門檻高、低優先)。
- **經濟層深化**:交易成本、換手率、容量、long-only 優化(空方既無效);regime-conditional(用 breadth/macro 當條件、非特徵)。
- **下一里程碑**:F3 模型層(本輪已建 `portfolio.py` 經濟回測基礎)。
