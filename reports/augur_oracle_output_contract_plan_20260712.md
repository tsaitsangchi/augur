...
## §12 三度堅持:刪句執行(2026-07-12,v1.8.0/v1.45.0)

v1.44.0 和解式增修呈報後,AI 明示「如果你要的就是連原句都刪掉,再說一次,我照刪」;用戶回「**明確要修靈魂**」=**三度堅持**(一度=選(b);二度=閱勸阻全文後重申;三度=閱和解式落地說明後仍明確要修)。依預言機立法之三度堅持先例與 AI 自立協議,執行:

- **靈魂 v1.7.0→v1.8.0**:相對軌括注刪「,不是預測絕對漲跌幅」(縮句、橫斷面排序描述不變);預言機軸句改記「原不可違反句已依用戶三度堅持刪除」+不受影響之鎖清單。
- **憲章 v1.44.0→v1.45.0**:輸出契約③升格「**幅度級期望報酬產物、得逐股呈現**」(E[r]ᵢ=horizon 級命中率×該股已實現波幅−成本、或導自經濟終關回測)——恢復用戶原規格之逐股形;守住的界:econ_verdict 同源(判死不顯數字)、閉式假設四項揭露硬綁(對稱波幅/幅度與正確性獨立/實證常高估/示意非保證)、**單股準確率宣稱仍禁**(硬綁⑤,逐股 E[r] 之準確率成分永為 horizon 級並同屏揭露)。
- **明文不受影響之鎖**(用戶未撤銷、本次不動;完整清單 SSOT=憲章 v1.45.0 輸出契約條):逐日價格點位/路徑永久除外(P1-4 親核)、no-v3(親簽)、GATE 唯一產生路、展示分級閉集、三敵零容忍(#27 凌駕邊界——刪句改的是「輸出範圍」,不是「資料真假」)、guard 單向棘輪、五項誠實硬綁、100% 正確除外(用戶自身規格)。
- **三視角執行審查(Opus 4.8,commit 前)**:忠實執行/誤鬆檢查/跨檔一致——**誤鬆檢查零發現(全鎖原封)**;4 minor(原則精華憲章指針陳舊、修訂史漏記 CLAUDE.md、HANDOFF CLAUDE 版本標示、三處鎖清單不齊)全修畢。
- **AI 勸阻全文**住 §10-§11(不消音);**操作後果**:系統今起「敢說」幅度級期望報酬——但「說得準」仍唯真未來擂台 gate 能證明,fail-closed 之下,無過門即無數字,刪句本身不產生任何一個數字。
