#!/usr/bin/env python
"""P0 治權隔離不變式 — 預測管線絕不 import philosophy/advisor(哲學絕不進預測管線)。

🎯 把憲章 v1.17.0「廣博哲學全文量化零價值、不進預測管線」從口號變成**可自動檢測的架構不變式**:
   features/models/universe/evaluation/ingestion/audit/catalog 任一模組 import
   `augur.philosophy` 或 `augur.advisor` = 違規 fail。哲學/顧問層只能單向依賴預測輸出、反向零依賴。
守原則 #1(零幻像)· #8(anti-leakage)· 憲章 v1.17.0 philosophy 邊界 · 學習計畫 P0(d) 隔離骨架。
...
FORBIDDEN = ("augur.philosophy", "augur.advisor")
...
def test_pipeline_does_not_import_philosophy_or_advisor():
    v = _violations()
    assert not v, "預測管線違反哲學隔離(哲學不進預測管線、憲章 v1.17.0):\n" + "\n".join(v)
...
if __name__ == "__main__":
    ...
    if v:
        print("❌ 違反哲學隔離:\n" + "\n".join(v))
        raise SystemExit(1)
    print(f"✓ 隔離不變式通過:{' / '.join(PIPELINE)} 皆未 import philosophy/advisor")
