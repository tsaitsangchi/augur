...
引用縮寫：**AUGUR-L5**｜版本：**v0.1-draft**（前版：無）
...
> ## 【地位】[N]
>
> 本文件為 **v0.1-draft 提案**，**未經 Constitution Steward（人類 tsaitsangchi）依 `AUGUR-MC v1.3 §8.1` 之充任認定前不生效力**。充任認定與 `§8.2` 實質合憲審查為 Steward 之裁決行為（`AUGUR-MC v1.3 §8.1`）；本規格起草人（Layer 5 子代理）**不宣稱任何規格已生效、不改本【地位】節自稱生效、不登錄正式 Amendment Log**。本文一切裁決一律標 **DRAFT**。
>
> * **生效要件**（`AUGUR-MC v1.3 §0.5`、`§0.1`）：`§0.5` 對照表 Layer 5 欄登錄（結構前提）＋ 依 `AUGUR-WM v1.0 §WM.39` 之 Compliance Statement（Annex CS）含 `§WM.44` 逐條矩陣（Annex TR）＋ linter 結構關卡（`§WM.40–43`）通過 ＋ **Steward 充任認定（未成就，DRAFT）** ＋ 登錄 Amendment Log（`§8.1`，屬 Steward 行為，未成就）。**殘餘生效阻卻為 Steward 充任認定未成就**。
> * **上層地位**：AUGUR-MC v1.3（L0）、AUGUR-WM v1.0（L1）、AUGUR-ONT v1.0（L2）、AUGUR-ID v1.0（L3）、AUGUR-KS v1.0（L4）均已生效；本規格對其之承接，於本規格自身生效後方對 Layer 6–7 產生規範效力。引用上層一律標其生效版本。
> * 本文件全部 [N] 條款於生效後對 Layer 6–7 規格產生規範效力；下層依 `AUGUR-L5 v{version} §{條款}` 格式引用。
