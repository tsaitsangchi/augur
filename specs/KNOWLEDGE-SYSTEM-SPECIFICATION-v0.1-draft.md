...
引用縮寫：**AUGUR-KS**｜版本：**v0.1-draft**（提案；前版：無）
...
> ## 【地位】[N]
>
> 本文件為 **AUGUR-KS v0.1-draft 提案版本**，**尚未生效**。依 `AUGUR-MC v1.3 §0.5`，任何後續規格必須先在 Layer 對照表登錄所屬 Layer 並經生效程序方生效力；`AUGUR-MC v1.3 §0.5` 對照表 Layer 4「Knowledge System」欄現登錄之所轄規格為「Knowledge Graph Specification、Data Intelligence Layer、原則精華」，本規格為該欄之概念層總綱規格，惟經 Steward 依 `AUGUR-MC v1.3 §8.1`／`§8.3` 過渡規則充任認定前，**不生效力**。在充任認定作成前：
>
> * 本文件全部 [N] 條款**僅具提案地位**，不對 Layer 5–7 規格產生規範效力，不得被下層引為定義依據；
> * **形式充分性（`AUGUR-WM v1.0 §WM.44`）之狀態**：`§WM.44` 要求 `AUGUR-MC v1.3`／`AUGUR-WM v1.0`／`AUGUR-ONT v0.1-draft`／`AUGUR-ID v0.1-draft` 全部 [N] 條款均對應至本規格至少一 [N] 條款、明記 DEFER 掛鉤、或明記「不觸及」及理由，且為機器可判之生效要件（「任一條款無對應且無明記者，聲明不完整，規格不生效力」）。本規格 Annex TR 已就四上層規格**全部 [N] 條款逐條完整枚舉**落點（TR.A：`§P4` 家族；TR.B：`AUGUR-MC v1.3` 非 P4；TR.C：`AUGUR-WM v1.0` WM.1–53＋Annex A＋Annex D；TR.D：`AUGUR-ONT v0.1-draft` ONT.1–62＋Annex T；TR.E：`AUGUR-ID v0.1-draft` ID.1–81＋IDO.0–8＋Annex L4），**形式充分性（`§WM.44`）已成就**。**殘餘之生效阻卻僅為 Steward 充任認定未成就**（`AUGUR-MC v1.3 §0.5`、`§8.3`）；上層草案升版或條款增修時本矩陣對應列必須同步維護（KS.111）；
...
> * 引用 Layer 2／Layer 3 一律標注其草案地位（`AUGUR-ONT v0.1-draft`、`AUGUR-ID v0.1-draft`）；上開草案本身未經充任認定前，本規格對其之承接於各該草案生效時同步生效；
> * 本文件之條款編號（KS.{n}、CM.{n}、EV.{n}、CL.{n}、KDI.{n}、KDO.{n}、TR.{n}、CS.{n}、EO.{n}）於本 draft 已宣告穩定性（KS.111），生效後準用 `AUGUR-MC v1.3 §8.6`、`AUGUR-WM v1.0 §WM.46`：永不重用、永不重排。
>
...
* 生效要件：`AUGUR-MC v1.3 §0.5` 對照表登錄（已具欄位）＋ Steward 充任認定（**未成就**，見【地位】）＋ 依 `AUGUR-WM v1.0 §WM.39` 之 Compliance Statement（Annex CS）含 `§WM.44` 逐條矩陣（**尚待補足**）＋ 登錄 Amendment Log（`AUGUR-MC v1.3 §8.1`）。
