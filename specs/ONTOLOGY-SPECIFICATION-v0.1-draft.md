...
引用縮寫：**AUGUR-ONT**｜版本：**v0.1-draft**（提案；前版：無）
...
> ## 【地位】[N]
>
> 本文件為 **v0.1-draft 提案版本**，**尚未生效**。依 `AUGUR-MC v1.3 §0.5`，任何後續規格必須先在 Layer 對照表登錄所屬 Layer 並經生效程序方生效力；`AUGUR-MC v1.3 §0.5` 對照表 Layer 2 欄已登錄「Ontology Specification」之欄位，惟本文件之**充任認定（Constitution Steward 依 `AUGUR-MC v1.3 §0.5`、`§8.6` 議決）尚未作成**。在充任認定作成前：
>
> * 本文件全部 [N] 條款**僅具提案地位**，不對 Layer 3–7 規格產生規範效力，不得被下層引為定義依據；
> * 本文件依 `AUGUR-MC v1.3 §8.3`、`AUGUR-WM v1.0 §WM.39` **必須內含**之 Constitutional Compliance Statement（Annex CS）以**正式格式**（`AUGUR-WM v1.0 §WM.39–45`，非暫行模板，`§WM.45`）作成，其充分性之最終判斷屬 Steward 違憲審查與充任認定程序（`AUGUR-MC v1.3 §8.2`、`§8.3`）；
> * 本文件之條款編號（ONT.{n}、T.{n}、DI.{n}、DO.{n}、EO.{n}、TM.{n}）於本 draft 已宣告穩定性（ONT.9），生效後準用 `AUGUR-MC v1.3 §8.6`、`AUGUR-WM v1.0 §WM.46`：永不重用、永不重排。
>
...
本附錄為資訊性對照，供 `AUGUR-WM v1.0 §WM.44` 形式充分性自查（Annex CS §CS.10）之機器盤點依據；規範效力悉依所引條款本文。模式閉集：滿足／細化（refines）／承接（carries）／DEFER（hooks）／不觸及（附理由）。
...
依 `§WM.44` 判準自查：`AUGUR-MC v1.3` **全部** [N] 條款（依 `§0.3` 條款編號系統枚舉）與 `AUGUR-WM v1.0` **全部** [N] 條款，均對應至本規格至少一條 [N] 條款、明記 DEFER 掛鉤（Annex DO、Annex DI）、或明記「不觸及」及理由——逐條對照見 **Annex TR**（TR.1 憲章側、TR.2 Layer 1 側）。明記「不觸及」之主要條款群及理由：
...
**實質充分性**由違憲審查程序（`AUGUR-MC v1.3 §8.2`）與充任認定（`§0.5`）判斷。
